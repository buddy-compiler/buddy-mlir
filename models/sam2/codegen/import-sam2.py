...
def _repair_exported(module_text):
    ...
    def _repl(m):
        ...
        fixes.append(
            (const, new_dims, new_outs, result, "tensor<" + outs + ">")
        )
        return (
            f'%{result} = "tosa.reshape"({op}, {const}) : '
    ...
    for const, new_dims, new_outs, result, old_outs in fixes:
        ...
        while True:
            ...
            seg = re.sub(
                r"(static_offsets = array<i64: )([0-9, ]+)",
                lambda mo: (
                    mo.group(1) + re.sub(r"\d+$", "0", mo.group(2).rstrip())
                ),
                seg,
            )
            seg = re.sub(
                r"(static_sizes = array<i64: )([0-9, ]+)",
                lambda mo, last=new_dims[-1]: (
                    mo.group(1)
                    + re.sub(r"\d+$", str(last), mo.group(2).rstrip())
                ),
                seg,
            )
...
print(
    "[import-sam2] Loading facebook/sam2-hiera-tiny (vision_encoder) from:",
    model_path,
)
m = AutoModel.from_pretrained(model_path, dtype=torch.float32).eval()
ve = m.vision_encoder
print(
    f"  vision_encoder: {type(ve).__name__}, "
...
print(
    f"[import-sam2] first graph: {len(params)} params, "
...
print(
    f"[import-sam2] Wrote forward.mlir, subgraph0.mlir, arg0.data to "
    f"{a.output_dir}"
)
