...
print("[SAM2-Import] Loading sam2-hiera-tiny (vision_encoder)...")
m=AutoModel.from_pretrained("facebook/sam2-hiera-tiny",dtype=torch.float32).eval()
ve=m.vision_encoder
print(f"  vision_encoder params: {sum(p.numel() for p in ve.parameters()):,}")
...
print(f"[SAM2-Import] 1 graph, {len(params)} params")
...
numpy.concatenate([p.detach().cpu().numpy().reshape([-1]) for p in ve.parameters()]).tofile(os.path.join(a.output_dir,"arg0.data"))
print("[SAM2-Import] Done!")
