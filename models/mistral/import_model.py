...
print(
    f"   hidden={model.config.hidden_size}, layers={model.config.num_hidden_layers}, kv_heads={model.config.num_key_value_heads}"
)
...
with open(os.path.join(layer_dir, "subgraph0_prefill.mlir"), "w") as f:
    print(driver_prefill.subgraphs[0]._imported_module, file=f)
with open(os.path.join(layer_dir, "forward_prefill.mlir"), "w") as f:
    print(driver_prefill.construct_main_graph(True), file=f)
with open(os.path.join(layer_dir, "subgraph0_decode.mlir"), "w") as f:
    print(driver_decode.subgraphs[0]._imported_module, file=f)
with open(os.path.join(layer_dir, "forward_decode.mlir"), "w") as f:
