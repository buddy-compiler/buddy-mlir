...
print("[ColBERT-Import] Loading colbertv2.0...")
m = AutoModel.from_pretrained(
    "colbert-ir/colbertv2.0", dtype=torch.float32
).eval()
...
print(f"[ColBERT-Import] 1 graph, {len(params)} params")
...
ld = os.path.join(a.output_dir, "layer_partitioned")
os.makedirs(ld, exist_ok=True)
with open(os.path.join(ld, "subgraph0.mlir"), "w") as f:
    print(dr.subgraphs[0]._imported_module, file=f)
with open(os.path.join(ld, "forward.mlir"), "w") as f:
    print(dr.construct_main_graph(True), file=f)
numpy.concatenate(
    [p.detach().cpu().numpy().reshape([-1]) for p in params]
).tofile(os.path.join(a.output_dir, "arg0.data"))
print("[ColBERT-Import] Done!")
