#!/usr/bin/env python3
# ===- partition_strategy.py - Weather-LLM layer split strategy -----------===//
#
...
# (as produced by the DeepSeek trace). Weather-LLM is a Llama-family decoder
# whose natural order is [logits,K0,V0,K1,V1,...], so the swap mangles the
# exported forward_prefill.mlir return order to [K0,logits,K1,V0,...], which no
# longer matches gen_session.py's PrefillABI ([kv0..kv{n-1}, logits]). The
# ModelSession::prefill then memcpy's maxTokenLen*vocabSize bytes out of the
# last 4MB KV slot, which segfaults. We restore the order gen_session.py expects
# by re-mapping the remap from "adjacent swap" to "move logits to the end".
...
def layer_split_strategy(kind: str) -> SplitStrategy:
    """Return the Weather-LLM (LlamaForCausalLM, 24 layers) layer split strategy.

    Weather-LLM-SFT is a dense Llama-family decoder (no MoE routing PowOp like
    ...
    if kind == "prefill":
        return SplitStrategy(
            name="weather_llm_prefill_layers",
            parallel_num=1,
            ops_count=[],
            stage_boundary_op=FlashAttentionForCpuPrefillOp,
            stage_boundary_op_num=24,
        )
    if kind == "decode":
        return SplitStrategy(
            name="weather_llm_decode_layers",
            parallel_num=1,
            ops_count=[],
            stage_boundary_op=GQAAttentionFusedOp,
            stage_boundary_op_num=24,
        )
    raise ValueError(f"unknown Weather-LLM layer split kind: {kind}")
