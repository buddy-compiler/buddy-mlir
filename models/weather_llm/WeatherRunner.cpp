...
//===----------------------------------------------------------------------===//
//
// Implements WeatherRunner::run().  This file owns session setup and
// dispatch:
//   - weight file loading
...
//
// Weather-LLM (AuraWorxAI/weather-llm-sft) is a LlamaForCausalLM with a custom
// SentencePiece tokenizer (tokenizer.model).  The in-tree SentencePiece-style
// tokenizer is tokenizeLlama()/revertLlama() in
// frontend/Interfaces/buddy/LLM/TextContainer.h; the vocab file it consumes is
// the token-per-line vocab.txt staged next to the .rax (see CMakeLists.txt).
//
...
namespace {
static constexpr int kEosToken = 3; // </s> (Llama EOS)
} // namespace
...
void WeatherRunner::run(const RunConfig &cfgIn) {
  ...
  // Weather-LLM uses a SentencePiece tokenizer; the in-tree Llama codec reads
  // the token-per-line vocab.txt staged alongside the .rax.
  TextCodec codec;
  codec.tokenize = [](Text<size_t, 2> &t, const std::string &vocab) {
    t.tokenizeLlama(vocab, BUDDY_WEATHER_LLM_MAX_TOKEN_LEN);
  };
  ...
  if (cfg.interactive) {
    ...
    buddy::ConversationManager conv(
        std::move(*chatTmpl), [&vocabPath](const std::string &text) -> size_t {
          ...
          tmp.tokenizeLlama(vocabPath, BUDDY_WEATHER_LLM_MAX_TOKEN_LEN);
