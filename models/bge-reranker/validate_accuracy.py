...
def generate_reference(output_dir: str):
    ...
    test_prompts = [
        ...
        ("the sky is blue", "weather is nice"),
    ]
    refs = {}
    for i, (a, b) in enumerate(test_prompts):
        ...
        with torch.no_grad():
            logits = (
                model(**inp).logits.detach().cpu().numpy().astype(np.float32)
            )
        refs[f"pair_{i}"] = {
            "text_a": a,
            "text_b": b,
...
def compare_outputs(reference_dir, buddy_output_dir, tolerance=1e-3):
    ...
    for key, ref in refs.items():
        ...
        if not os.path.exists(bp):
            ...
            continue
        bl = np.load(bp)
        rl = np.load(os.path.join(reference_dir, f"ref_logits_{idx}.npy"))
        err = float(np.max(np.abs(rl - bl)))
        passed = err < tolerance
        print(
            f"  {'PASS' if passed else 'FAIL'} | {ref['text_a'][:30]} | max_err={err:.2e}"
        )
    return True
...
def main():
    ...
    if a.mode == "reference":
        ...
        sys.exit(
            0
            if compare_outputs(a.reference_dir, a.buddy_output_dir, a.tolerance)
            else 1
        )
