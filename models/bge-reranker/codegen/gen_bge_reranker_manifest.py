#!/usr/bin/env python3
# ===- gen_bge_reranker_manifest.py - RHAL manifest for BGE-Reranker -------===//
#
...
#
# Usage:
#   python gen_bge_reranker_manifest.py --spec specs/f32.json \
#       -o bge_reranker.mlir --runner-library bge_reranker_runner.so
#
# The buffer/function layout mirrors the AOT forward ABI produced by
# codegen/import-bge-reranker.py (XLMRobertaForSequenceClassification):
...
#     -> (logits: memref<1 x num_labels x f32>)
#
# ===----------------------------------------------------------------------===//
...
def gen_manifest(spec: dict, runner_library: str) -> str:
    ...
    num_labels = int(spec.get("num_labels", 1))
    ...
    p('    args     = ["input_ids", "attention_mask", "position_ids",')
    p('                "logits"]}')
    p("}")
    return "\n".join(lines) + "\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate RHAL manifest for the BGE-Reranker model."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--runner-library",
        default="bge_reranker_runner.so",
