...
//===----------------------------------------------------------------------===//
//
// BGE-Reranker-v2-M3's tokenizer is XLM-RoBERTa's: a SentencePiece Unigram
// model with a Metaspace pre-tokenizer. This loads it straight from the
// HuggingFace `tokenizers` fast-tokenizer export (tokenizer.json) and
// reproduces
//   AutoTokenizer(query, document, padding="max_length", truncation=True,
//                 max_length=N)
// without a Python/`transformers` runtime dependency. The encoder
// (XLMRobertaForSequenceClassification) consumes the pair as a single
// sequence:
//   [<s>, query..., </s>, document..., </s>]
// which is what `encodePair` below produces.
//
...
#ifndef BUDDY_RUNTIME_MODELS_BGE_RERANKER_TOKENIZER_H
#define BUDDY_RUNTIME_MODELS_BGE_RERANKER_TOKENIZER_H
...
public:
  static BgeRerankerTokenizer loadFromFile(const std::string &tokenizerJsonPath) {
    ...
    if (!parsed)
      throw std::runtime_error("BgeRerankerTokenizer: failed to parse " +
                               tokenizerJsonPath + ": " +
                               llvm::toString(parsed.takeError()));

    const llvm::json::Object *root = parsed->getAsObject();
    if (!root)
      throw std::runtime_error("BgeRerankerTokenizer: " + tokenizerJsonPath +
                               " root is not a JSON object");

    BgeRerankerTokenizer tok;
    tok.loadVocab(*root);
  ...
  // Single-text encoding: [<s>, content..., </s>], right-padded.
  void encode(const std::string &text, size_t maxSeqLen,
              std::vector<int64_t> &inputIds,
              std::vector<int64_t> &attentionMask) const {
    if (maxSeqLen < 2)
      throw std::runtime_error("BgeRerankerTokenizer: maxSeqLen must be >= 2");
  ...
  // Cross-encoder pair encoding for the reranker, matching
  //   tokenizer(query, document, padding="max_length", truncation=True,
  //             max_length=maxSeqLen)
  // => [<s>, query..., </s>, document..., </s>], right-padded with padId_ /
  // mask 0. Truncation uses HF's default "longest_first" strategy: the longer
  // side is trimmed first until the wrapped sequence fits maxSeqLen.
  void encodePair(const std::string &query, const std::string &document,
                  size_t maxSeqLen, std::vector<int64_t> &inputIds,
                  std::vector<int64_t> &attentionMask) const {
    if (maxSeqLen < 3)
      throw std::runtime_error("BgeRerankerTokenizer: maxSeqLen must be >= 3");

    std::vector<int64_t> q = tokenize(query);
    std::vector<int64_t> d = tokenize(document);

    // budget covers [<s>, q..., </s>, d..., </s>] (3 special tokens).
    size_t budget = maxSeqLen - 3;
    while (q.size() + d.size() > budget) {
      if (q.size() >= d.size() && !q.empty())
        q.pop_back();
      else if (!d.empty())
        d.pop_back();
      else
    ...
    inputIds.push_back(bosId_);
    inputIds.insert(inputIds.end(), q.begin(), q.end());
    inputIds.push_back(eosId_);
    inputIds.insert(inputIds.end(), d.begin(), d.end());
    inputIds.push_back(eosId_);
    attentionMask.assign(inputIds.size(), 1);
...
private:
  ...
  void loadVocab(const llvm::json::Object &root) {
    ...
    if (!model)
      throw std::runtime_error("BgeRerankerTokenizer: missing \"model\"");
    auto type = model->getString("type");
    if (!type || *type != "Unigram")
      throw std::runtime_error(
          "BgeRerankerTokenizer: unsupported tokenizer model type (expected "
          "Unigram)");
    ...
    if (!vocabArr)
      throw std::runtime_error("BgeRerankerTokenizer: missing \"model.vocab\"");
    vocab_.reserve(vocabArr->size());
    for (const llvm::json::Value &entry : *vocabArr) {
      const llvm::json::Array *pair = entry.getAsArray();
      if (!pair || pair->size() != 2)
        throw std::runtime_error("BgeRerankerTokenizer: malformed vocab entry");
  ...
  void loadCharsmap(const llvm::json::Object &root) {
    ...
    for (const llvm::json::Object *step : flat) {
      ...
      if (llvm::Error err = llvm::decodeBase64(*b64, charsmap_))
        throw std::runtime_error(
            "BgeRerankerTokenizer: failed to decode precompiled_charsmap: " +
    ...
    if (charsmap_.size() < sizeof(uint32_t))
      throw std::runtime_error("BgeRerankerTokenizer: precompiled_charsmap too "
                               "small");
    ...
    if (static_cast<size_t>(xcdaBlobSize) + offset > charsmap_.size())
      throw std::runtime_error(
          "BgeRerankerTokenizer: precompiled_charsmap XCDA blob out of bounds");
  ...
  NormalizedPrefix normalizePrefix(const std::string &input,
    ...
    if (longestLen > 0) {
      if (longestReplOffset >= prefixReplacementsSize_)
        throw std::runtime_error(
            "BgeRerankerTokenizer: charsmap replacement offset out of bounds");
      const char *repl =
...
#endif // BUDDY_RUNTIME_MODELS_BGE_RERANKER_TOKENIZER_H
