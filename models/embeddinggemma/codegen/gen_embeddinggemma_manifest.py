#!/usr/bin/env python3
# ===- gen_embeddinggemma_manifest.py - RHAL manifest for embeddinggemma -===//
#
...
#
# Usage:
#   python gen_embeddinggemma_manifest.py --spec specs/f32.json -o embeddinggemma.mlir
#
...
def gen_manifest(spec: dict, runner_library: str) -> str:
    ...
    max_position_embeddings = int(spec.get("max_position_embeddings", 2048))
    ...
    p('    args     = ["input_ids", "attention_mask", "embedding"]}')
    p("}")
    return "\n".join(lines) + "\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate RHAL manifest for the embeddinggemma model."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--runner-library",
        default="embeddinggemma_runner.so",
