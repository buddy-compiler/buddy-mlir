#!/usr/bin/env python3
# ===- partition_strategy.py - Qwen3 layer split strategy ------------------===//
#
...
# (as produced by the DeepSeek trace). Qwen3 is a Llama-family decoder whose
...
# matches gen_session.py's PrefillABI ([kv0..kv55, logits]). ModelSession::prefill
# then memcpy's cfg_.maxTokenLen*vocabSize bytes out of the last 4MB KV slot,
...
# DecodeABI interleaves a dummy input between each (K,V) pair (27 dummies for 56
# KV tensors) and also returns cachePositionOut + the dummies, while the generic
# driver emits a contiguous 59-arg / 57-return decode wrapper. We rewrite the
# text of the emitted forward_decode.mlir wrapper to match DecodeABI.
...
def _transform_decode_text(module_text):
    ...
    @subgraph0_decode0 keeps its contiguous signature.  Everything (n, KV type,
    logits type) is derived from the module text, so this works for any Llama
...
def layer_split_strategy(kind: str) -> SplitStrategy:
    """Return the Qwen3 (Qwen3ForCausalLM, 28 layers) layer split strategy.

    Qwen3-0.6B is a dense Llama-family decoder (no MoE routing PowOp like
    DeepSeek R1). The fusion pass produces exactly one fused attention op per
    decoder layer -- FlashAttentionForCpuPrefillOp in the prefill graph (after
    `flash_attention_prefill`) and GQAAttentionFusedOp in the decode graph
    (after `gqa_attention_fusion`) -- so a vertical split on those boundaries
    yields one subgraph per layer. parallel_num stays 1: weights and tensor
    shapes are never horizontally sharded.
    """
    if kind == "prefill":
        return SplitStrategy(
            name="qwen3_prefill_layers",
            parallel_num=1,
            ops_count=[],
            stage_boundary_op=FlashAttentionForCpuPrefillOp,
            stage_boundary_op_num=28,
        )
    if kind == "decode":
        return SplitStrategy(
            name="qwen3_decode_layers",
            parallel_num=1,
            ops_count=[],
            stage_boundary_op=GQAAttentionFusedOp,
            stage_boundary_op_num=28,
        )
    raise ValueError(f"unknown Qwen3 layer split kind: {kind}")
