//===- Qwen3Runner.cpp - Qwen3 full inference loop ------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// Implements Qwen3Runner::run().  This file owns session setup and dispatch:
//   - weight file loading
...
void Qwen3Runner::run(const RunConfig &cfgIn) {
  RunConfig cfg = cfgIn;
  ...
  printLog("KV cache: " + std::to_string(BUDDY_QWEN3_KV_LAYERS) + " x {1," +
               std::to_string(BUDDY_QWEN3_HEAD_NUM) + "," +
               std::to_string(BUDDY_QWEN3_MAX_TOKEN_LEN) + "," +
               std::to_string(BUDDY_QWEN3_HIDDEN_SIZE) + "} f32",
           suppress);

  TextCodec codec;
  codec.tokenize = [](Text<size_t, 2> &t, const std::string &vocab) {
    t.tokenizeQwen3(vocab, BUDDY_QWEN3_MAX_TOKEN_LEN);
  };
  codec.detokenize = [](Text<size_t, 2> &t) { return t.revertQwen3(); };
  codec.maxTokenLen = BUDDY_QWEN3_MAX_TOKEN_LEN;
