#!/usr/bin/env python3
# ===- gen_e5_mistral_manifest.py - RHAL manifest for E5-Mistral -----------===//
#
...
#
# Usage:
#   python gen_e5_mistral_manifest.py --spec specs/f32.json \
#       -o e5_mistral.mlir --runner-library e5_mistral_runner.so
#
# The buffer/function layout mirrors the AOT forward ABI produced by
# codegen/import-e5-mistral.py (a Mistral-architecture encoder, single result):
#
...
def gen_manifest(spec: dict, runner_library: str) -> str:
    ...
    p(
        f'    max_position_embeddings = "{spec.get("max_position_embeddings", 32768)}",'
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate RHAL manifest for the E5-Mistral model."
    )
    parser.add_argument(
    ...
    if args.output == "-":
        ...
        with open(args.output, "w") as f:
            f.write(text)
        print(
            f"[gen_e5_mistral_manifest] Written: {args.output}", file=sys.stderr
        )
    return 0
