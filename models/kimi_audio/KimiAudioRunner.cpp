//===- KimiAudioRunner.cpp - Kimi-Audio full inference loop ----------------===//
//
...
//===----------------------------------------------------------------------===//
//
// Kimi-Audio-7B-Instruct inference runner.
// Based on Qwen2.5-7B with dual text+audio output heads and MIMO layers.
//
//===----------------------------------------------------------------------===//
...
static constexpr int kEosToken2 = 151644; // <|endoftext|>
} // namespace

void KimiAudioRunner::run(const RunConfig &cfgIn) {
  RunConfig cfg = cfgIn;
  ...
  if (!suppress)
    std::cerr
        << "\033[35;1mKimi-Audio Inference (buddy-cli / BuddyRuntime)\033[0m\n";

  // ── Chat template ───────────────────────────────────────────────────────
  std::vector<long long> stopTokenIds = {kEosToken, kEosToken2};
  ...
  printLog("KV cache: " + std::to_string(BUDDY_KIMI_AUDIO_KV_LAYERS) +
               " x {1," + std::to_string(BUDDY_KIMI_AUDIO_HEAD_NUM) + "," +
               std::to_string(BUDDY_KIMI_AUDIO_MAX_TOKEN_LEN) + "," +
               std::to_string(BUDDY_KIMI_AUDIO_HIDDEN_SIZE) + "} f32",
           suppress);

  // ── Text codec (Qwen-based tokenizer) ───────────────────────────────────
  TextCodec codec;
  codec.tokenize = [](Text<size_t, 2> &t, const std::string &vocab) {
    t.tokenizeQwen3(vocab, BUDDY_KIMI_AUDIO_MAX_TOKEN_LEN);
  };
  codec.detokenize = [](Text<size_t, 2> &t) { return t.revertQwen3(); };
  codec.maxTokenLen = BUDDY_KIMI_AUDIO_MAX_TOKEN_LEN;
  ...
  if (cfg.interactive) {
    ...
    buddy::ConversationManager conv(
        std::move(*chatTmpl), [&](const std::string &text) -> size_t {
          ...
          tmp.tokenizeQwen3(vocabPath, BUDDY_KIMI_AUDIO_MAX_TOKEN_LEN);
