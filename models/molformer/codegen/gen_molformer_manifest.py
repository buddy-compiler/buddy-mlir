#!/usr/bin/env python3
# ===- gen_molformer_manifest.py - RHAL manifest for MoLFormer ------------===//
#
...
#
# Usage:
#   python gen_molformer_manifest.py --spec specs/f32.json -o molformer.mlir
#
# The buffer/function layout mirrors the AOT forward ABI produced by
# codegen/import-molformer.py:
...
def gen_manifest(spec: dict, runner_library: str) -> str:
    ...
    p(
        f'  rhal.buffer @pooled {{space = "host", '
        f"type = tensor<1x{hidden_size}xf32>}}"
    )
    p("")
    p("  rhal.func @forward {")
    p('    inputs   = ["input_ids", "attention_mask"],')
    p('    outputs  = ["last_hidden_state", "pooled"],')
    p('    dispatch = "model_kernels",')
    p('    args     = ["input_ids", "attention_mask",')
    p('                "last_hidden_state", "pooled"]}')
    p("}")
    return "\n".join(lines) + "\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate RHAL manifest for the MoLFormer model."
    )
    parser.add_argument(
    ...
    if args.output == "-":
        ...
        with open(args.output, "w") as f:
            f.write(text)
        print(
            f"[gen_molformer_manifest] Written: {args.output}", file=sys.stderr
        )
    return 0
