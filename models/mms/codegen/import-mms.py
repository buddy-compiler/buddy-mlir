...
def _postprocess_subgraph0(text):
    ...
    def build_phases(s, pfx, ind="    "):
        ...
        G = []
        G.append(
            f'{ind}%{pfx}_s4 = "tosa.const_shape"() <{{values = dense<[1, {Cout}, {L}, 1]> : tensor<4xindex>}}> : () -> !tosa.shape<4>'
        )
        G.append(
            f'{ind}%{pfx}_sout = "tosa.const_shape"() <{{values = dense<[1, {Cout}, {N}]> : tensor<3xindex>}}> : () -> !tosa.shape<3>'
        )
        G.append(
            f'{ind}%{pfx}_front = "tosa.const"() <{{values = dense<0.000000e+00> : tensor<1x{Cin}x{PF}xf32>}}> : () -> tensor<1x{Cin}x{PF}xf32>'
        )
        G.append(
            f'{ind}%{pfx}_back = "tosa.const"() <{{values = dense<0.000000e+00> : tensor<1x{Cin}x{PB}xf32>}}> : () -> tensor<1x{Cin}x{PB}xf32>'
        )
        G.append(
            f'{ind}%{pfx}_xp = "tosa.concat"(%{pfx}_front, {srcv}, %{pfx}_back) <{{axis = 2 : i32}}> : (tensor<1x{Cin}x{PF}xf32>, {sty}, tensor<1x{Cin}x{PB}xf32>) -> tensor<1x{Cin}x{Lxp}xf32>'
        )
        G.append(
            f'{ind}%{pfx}_wt = "tosa.transpose"(%{s["weight"]}) <{{perms = array<i32: 1, 0, 2>}}> : ({wty}) -> tensor<{Cout}x{Cin}x{K}xf32>'
        )
        G.append(
            f'{ind}%{pfx}_stack = "tensor.empty"() : () -> tensor<1x{Cout}x{L}x{S}xf32>'
        )
        G.append(
            f'{ind}%{pfx}_z = "arith.constant"() <{{value = 0.000000e+00 : f32}}> : () -> f32'
        )
        prev = f"%{pfx}_stack"
        for r in range(S):
            k_r = (r + P) % S
            ...
            k_last = k_r + (T - 1) * S
            G.append(
                f'{ind}%{pfx}_wr_{r} = "tensor.extract_slice"(%{pfx}_wt) <{{operandSegmentSizes = array<i32: 1, 0, 0, 0>, static_offsets = array<i64: 0, 0, {k_last}>, static_sizes = array<i64: {Cout}, {Cin}, {T}>, static_strides = array<i64: 1, 1, -{S}>}}> : (tensor<{Cout}x{Cin}x{K}xf32>) -> tensor<{Cout}x{Cin}x{T}xf32>'
            )
            G.append(
                f'{ind}%{pfx}_e_{r} = "tensor.empty"() : () -> tensor<1x{Cout}x{LC}xf32>'
            )
            G.append(
                f'{ind}%{pfx}_f_{r} = "linalg.fill"(%{pfx}_z, %{pfx}_e_{r}) <{{operandSegmentSizes = array<i32: 1, 1>}}> ({{'
            )
            G.append(f"{ind}^bb0(%{pfx}a{r}: f32, %{pfx}b{r}: f32):")
            G.append(f'{ind}  "linalg.yield"(%{pfx}a{r}) : (f32) -> ()')
            G.append(
                f"{ind}}}) : (f32, tensor<1x{Cout}x{LC}xf32>) -> tensor<1x{Cout}x{LC}xf32>"
            )
            G.append(
                f'{ind}%{pfx}_C_{r} = "linalg.conv_1d_ncw_fcw"(%{pfx}_xp, %{pfx}_wr_{r}, %{pfx}_f_{r}) <{{dilations = dense<1> : tensor<1xi64>, operandSegmentSizes = array<i32: 2, 1>, strides = dense<1> : tensor<1xi64>}}> ({{'
            )
            G.append(
                f"{ind}^bb0(%{pfx}x{r}: f32, %{pfx}y{r}: f32, %{pfx}w{r}: f32):"
            )
            G.append(
                f'{ind}  %{pfx}m{r} = "arith.mulf"(%{pfx}x{r}, %{pfx}y{r}) <{{fastmath = #arith.fastmath<none>}}> : (f32, f32) -> f32'
            )
            G.append(
                f'{ind}  %{pfx}d{r} = "arith.addf"(%{pfx}w{r}, %{pfx}m{r}) <{{fastmath = #arith.fastmath<none>}}> : (f32, f32) -> f32'
            )
            G.append(f'{ind}  "linalg.yield"(%{pfx}d{r}) : (f32) -> ()')
            G.append(
                f"{ind}}}) : (tensor<1x{Cin}x{Lxp}xf32>, tensor<{Cout}x{Cin}x{T}xf32>, tensor<1x{Cout}x{LC}xf32>) -> tensor<1x{Cout}x{LC}xf32>"
            )
            G.append(
                f'{ind}%{pfx}_or_{r} = "tensor.extract_slice"(%{pfx}_C_{r}) <{{operandSegmentSizes = array<i32: 1, 0, 0, 0>, static_offsets = array<i64: 0, 0, {delta + PX}>, static_sizes = array<i64: 1, {Cout}, {L}>, static_strides = array<i64: 1, 1, 1>}}> : (tensor<1x{Cout}x{LC}xf32>) -> tensor<1x{Cout}x{L}xf32>'
            )
            G.append(
                f'{ind}%{pfx}_o4_{r} = "tosa.reshape"(%{pfx}_or_{r}, %{pfx}_s4) : (tensor<1x{Cout}x{L}xf32>, !tosa.shape<4>) -> tensor<1x{Cout}x{L}x1xf32>'
            )
            G.append(
                f'{ind}%{pfx}_st_{r} = "tensor.insert_slice"(%{pfx}_o4_{r}, {prev}) <{{operandSegmentSizes = array<i32: 1, 1, 0, 0, 0>, static_offsets = array<i64: 0, 0, 0, {r}>, static_sizes = array<i64: 1, {Cout}, {L}, 1>, static_strides = array<i64: 1, 1, 1, 1>}}> : (tensor<1x{Cout}x{L}x1xf32>, tensor<1x{Cout}x{L}x{S}xf32>) -> tensor<1x{Cout}x{L}x{S}xf32>'
            )
            prev = f"%{pfx}_st_{r}"
        G.append(
            f'{ind}{s["name"]} = "tosa.reshape"({prev}, %{pfx}_sout) : (tensor<1x{Cout}x{L}x{S}xf32>, !tosa.shape<3>) -> tensor<1x{Cout}x{N}xf32>'
        )
        return G
    ...
    for idx, s in enumerate(sites):
        replacements.append(
            (s["const_shape"], s["end"], build_phases(s, f"ph{idx}"))
        )
...
print(
    f"[import-mms] Wrote subgraph0.mlir, forward.mlir, arg0.data "
