#!/usr/bin/env python3
# ===- gen_chemberta_manifest.py - RHAL manifest for ChemBERTa ------------===//
#
...
#
# Usage:
#   python gen_chemberta_manifest.py --spec specs/f32.json -o chemberta.mlir
#
# The buffer/function layout mirrors the AOT forward ABI produced by
# codegen/import-chemberta.py:
...
#
# `position` is the flattened position_ids buffer consumed by the position
# embedding gather (size == max_position_embeddings for this RoBERTa-style
# encoder).
#
# ===----------------------------------------------------------------------===//
...
def gen_manifest(spec: dict, runner_library: str) -> str:
    ...
    p(f'    position_buffer_size = "{position_buffer_size}",')
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate RHAL manifest for the ChemBERTa model."
    )
    parser.add_argument(
