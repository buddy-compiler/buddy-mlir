#!/usr/bin/env python3
# ===- gen_paddleocr_manifest.py - RHAL manifest for PaddleOCR-VL -----------===//
#
...
#
# Usage:
#   python gen_paddleocr_manifest.py --spec specs/f32.json -o paddleocr.mlir \
#       --runner-library paddleocr_runner.so
#
# The buffer/function layout mirrors the AOT forward ABI produced by
# codegen/import-paddleocr.py (fixed-shape OCR sequence of max_seq_len tokens,
# of which the first 972 are image tokens):
#
#   forward(weights: memref<params_size x f32>,
#           input_ids: memref<1 x max_seq_len x i64>,
#           pixel_values: memref<num_image_patches x 3 x patch_size x patch_size x f32>,
#           attention_mask: memref<1 x max_seq_len x i64>,
#           position_ids: memref<3 x 1 x max_seq_len x i64>)
...
def gen_manifest(spec: dict, runner_library: str) -> str:
    ...
    num_image_patches = int(spec.get("num_image_patches", 3888))
    patch_size = int(spec.get("patch_size", 14))
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate RHAL manifest for the PaddleOCR-VL model."
    )
    parser.add_argument(
