...
// You may obtain N copy of the License at
//
...
public:
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> /*operands*/,
    ...
    Value OC = rewriter.create<memref::DimOp>(loc, output, 1); // OC
    Value OH = rewriter.create<memref::DimOp>(loc, output, 2); // OH
    Value OW = rewriter.create<memref::DimOp>(loc, output, 3); // OW
    Value IC = rewriter.create<memref::DimOp>(loc, input, 1);  // IC
    Value FH = rewriter.create<memref::DimOp>(loc, filter, 2); // FH
    Value FW = rewriter.create<memref::DimOp>(loc, filter, 3); // FW
