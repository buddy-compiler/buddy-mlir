//===- MatMulParallelVectorizationTiling.cpp ------------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements matmul parallel vectorization with k-block tiling.
//
//===----------------------------------------------------------------------===//
...
public:
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> /*operands*/,
    ...
    Type elementType = cast<MemRefType>(A.getType()).getElementType();
    ...
    if (cast<MemRefType>(C.getType()).isDynamicDim(1) ||
...
//===----------------------------------------------------------------------===//
// MatMulParallelVectorizationTilingPass
//===----------------------------------------------------------------------===//

/// This pass partially lowers linalg.matmul to a mixture of
/// Affine + Vector operations with k-block tiling.
namespace {
class MatMulParallelVectorizationTilingPass
    : public PassWrapper<MatMulParallelVectorizationTilingPass,
...
public:
  ...
  StringRef getArgument() const final {
    return "matmul-parallel-vectorization-tiling";
  }

  StringRef getDescription() const final {
    return "MatMul parallel vectorization with k-block tiling optimization.";
  }

  MatMulParallelVectorizationTilingPass() = default;
  MatMulParallelVectorizationTilingPass(
      const MatMulParallelVectorizationTilingPass &) {}

  explicit MatMulParallelVectorizationTilingPass(
      int64_t affineVectorSizeParam, int64_t kBlockSizeParam) {
    affineVectorSize = affineVectorSizeParam;
    kBlockSize = kBlockSizeParam;
  }
...
void MatMulParallelVectorizationTilingPass::runOnOperation() {
...
void registerMatMulParallelVectorizationTilingPass() {
  PassRegistration<MatMulParallelVectorizationTilingPass>();
}
} // namespace buddy
} // namespace mlir