...
//===----------------------------------------------------------------------===//
//
// This file defines upper level operators scheduling on different hardwares
// using sche dialect pass.
//
//===----------------------------------------------------------------------===//
...
public:
  ...
  ValueRange getReturnValues() { return return_; }

  ValueRange getOperands() { return operands_; }

  StringRef getTargetId() { return target_id_; }

  StringRef getTargetConfig() { return target_config_; }

  SmallVector<Operation *> getOpList() { return op_list_; }

  void setOpList(SmallVector<Operation *> op_list) { op_list_ = op_list; }
  ...
  void updateDefinedValue() {
    for (auto &&op : op_list_) {
      for (auto &&result : op->getResults()) {
        if (std::find(defined_.begin(), defined_.end(), result) ==
            defined_.end())
  ...
  void updateReturnValue() {
    for (auto &&v : defined_) {
      for (auto &&user : v.getUsers()) {
        ...
        if (std::find(op_list_.begin(), op_list_.end(), user) ==
            op_list_.end()) {
          if (std::find(return_.begin(), return_.end(), v) == return_.end())
            return_.push_back(v);
          break;
    ...
    rewriter.modifyOpInPlace(func, [&]() {
      ...
      for (auto &&node : sche_target_node_list) {
        ...
        auto on_device_op = rewriter.create<sche::OnDeviceOp>(
            loc, node->getTargetId(), node->getTargetConfig(),
            node->getReturnValues().getTypes(), operands,
        ...
        on_device_op.getOperation()->setAttr("sche.source",
                                             rewriter.getStringAttr("func"));

        for (auto &&[a, b] :
             llvm::zip(node->getReturnValues(), on_device_op.getResults())) {
...
public:
  ...
  LogicalResult matchAndRewrite(scf::ForOp forOp,
    ...
    auto devices =
        dyn_cast_or_null<ArrayAttr>(op->getAttr("sche.devices")).getValue();
    ...
    stepRange = rewriter.create<arith::IndexCastOp>(loc, rewriter.getI32Type(),
                                                    stepRange);
    stepRange =
        rewriter.create<arith::SIToFPOp>(loc, rewriter.getF32Type(), stepRange);
    auto start = lowerBound;
    auto end = lowerBound;

    // Build for loops on different hardware, requiring the last device to have
    // the highest load
    for (auto i = 0; i < devices.size(); i++) {
      ...
      assert(isa<StringAttr>(targetId) && isa<StringAttr>(targetConfig) &&
             isa<FloatAttr>(dict_attr.get("duty_ratio")));

      rewriter.setInsertionPoint(placeHolder);
      // The upperBound of the last for loop is the original upperBound, and the
      // previous for loop is rounded towards zero
      if (i == devices.size() - 1) {
        end = upperBound;
      } else {
        auto duty_ratio = rewriter.create<arith::ConstantOp>(
            loc, rewriter.getF32Type(),
            dict_attr.get("duty_ratio").dyn_cast<FloatAttr>());
        Value duty_value = rewriter.create<arith::MulFOp>(
            loc, stepRange, duty_ratio.getResult());
        duty_value = rewriter.create<arith::FPToSIOp>(
            loc, rewriter.getI32Type(), duty_value);
        duty_value = rewriter.create<arith::IndexCastOp>(
            loc, rewriter.getIndexType(), duty_value);
      ...
      if (targetId.dyn_cast<StringAttr>().getValue() == "cpu") {
        ...
        auto sub_forOp = rewriter.create<scf::ForOp>(
            loc, start, end, step, forOp.getInitArgs(),
        ...
        auto on_device_op = rewriter.create<sche::OnDeviceOp>(
            ...
            forOp.getResults().getTypes(), operands_,
            [&](OpBuilder &builder, Location loc, ValueRange valueRange) {
              auto sub_forOp = builder.create<scf::ForOp>(
                  loc, start, end, step, forOp.getInitArgs(),
                  [&](OpBuilder &builder, Location loc, Value iv,
                      ValueRange iterArgs) {
                    IRMapping mp;
                    mp.map(bodyBlock.getArgument(0), iv);
                    for (auto &&[a, b] : llvm::zip(
                             bodyBlock.getArguments().drop_front(), iterArgs)) {
                      mp.map(a, b);
                    }
                    for (auto &&op_ : bodyBlock.getOperations()) {
                      builder.insert(op_.clone(mp));
        ...
        on_device_op.getOperation()->setAttr("sche.source",
                                             rewriter.getStringAttr("scf.for"));

        rewriter.setInsertionPointAfter(op);
        rewriter.create<sche::WaitOp>(loc, (Type) nullptr,
                                      ValueRange{on_device_op.getAsyncToken()});
      }
...
class ReduceSumOpDeviceSchedulePattern
    : public OpRewritePattern<tosa::ReduceSumOp> {
public:
  using OpRewritePattern<tosa::ReduceSumOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(tosa::ReduceSumOp reduceSumOp,
                                PatternRewriter &rewriter) const override {
    ...
    auto split_axis =
        dyn_cast_or_null<IntegerAttr>(op->getAttr("sche.axis")).getInt();
    ...
    for (auto i = 0; i < devices.size(); i++) {
      ...
      if (i == devices.size() - 1) {
        ...
        auto duty_ratio = dict_attr.get("duty_ratio")
                              .dyn_cast<FloatAttr>()
                              .getValueAsDouble();
      ...
      Value result = rewriter.create<tosa::SliceOp>(
          loc,
          RankedTensorType::get(size_shape, input_tensor_type.getElementType()),
          input, start_shape, size_shape);
      if (targetId.dyn_cast<StringAttr>().getValue() == "cpu") {
        ...
        auto on_device_op = rewriter.create<sche::OnDeviceOp>(
            ...
            TypeRange{RankedTensorType::get(
                result_shape, input_tensor_type.getElementType())},
            ValueRange{},
            [&](OpBuilder &builder, Location loc, ValueRange valueRange) {
              result = rewriter.create<tosa::ReduceSumOp>(
                  loc,
    ...
    if (reduce_axis == split_axis) {
      for (int i = 1; i < inter_results.size(); i++) {
        res = rewriter.create<tosa::AddOp>(loc, res.getType(), res,
                                           inter_results[i]);
      }
    } else {
      for (int i = 1; i < inter_results.size(); i++) {
        res = rewriter.create<tosa::ConcatOp>(
            loc, ValueRange{res, inter_results[i]}, split_axis);
      }
    }
...
namespace {
class DeviceSchedulePass
    : public PassWrapper<DeviceSchedulePass, OperationPass<ModuleOp>> {
public:
...
void DeviceSchedulePass::runOnOperation() {
  ...
  target.addDynamicallyLegalOp<func::FuncOp>(
      [](Operation *op) { return !op->hasAttr("sche.devices"); });
  target.addDynamicallyLegalOp<scf::ForOp, tosa::ReduceSumOp>(
      [&](Operation *op) { return !op->hasAttr("sche.devices"); });
