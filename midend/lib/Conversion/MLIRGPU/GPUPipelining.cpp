...
static void setAsyncAnnotations(Operation *op,
                                ...
                                buddy::PipeliningSchedulingStrategy schedule) {
  if (auto waitOp = dyn_cast<nvgpu::DeviceAsyncWaitOp>(op)) {
    ...
    // copies in flight.
    bool copyBeforeLoad =
        schedule == buddy::PipeliningSchedulingStrategy::nvidiaTensorCore;
    ...
    unsigned pipelineStoreStage =
        schedule == buddy::PipeliningSchedulingStrategy::loadStoreStage0 ? 0 : 1;
...
applyPipelining(scf::ForOp forOp, int64_t depth, bool epiloguePeeling,
  ...
  if (schedule == buddy::PipeliningSchedulingStrategy::loadGlobalStage0 ||
      schedule == buddy::PipeliningSchedulingStrategy::loadStoreStage0) {
    unsigned pipelineStoreStage =
        schedule == buddy::PipeliningSchedulingStrategy::loadGlobalStage0;
  ...
  auto getSchedule =
      [maxDepth, schedule](scf::ForOp forOp,
                           std::vector<std::pair<Operation *, unsigned>> &ops) {
        if (schedule == buddy::PipeliningSchedulingStrategy::nvidiaTensorCore) {
...
//       schedule = (buddy::PipeliningSchedulingStrategy)
...
pipelineSharedMemoryCopy(RewriterBase &rewriter, scf::ForOp forOp,
                         buddy::PipeliningSchedulingStrategy strategy,
