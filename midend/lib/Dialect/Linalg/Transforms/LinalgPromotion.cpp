...
allocateWorkgroupMemory(OpBuilder &builder, memref::SubViewOp subview,
  ...
  auto smemBufferType = MemRefType::get(
      shapes, subview.getType().getElementType(), MemRefLayoutAttrInterface{},
      gpu::AddressSpaceAttr::get(builder.getContext(),
                                 gpu::GPUDialect::getWorkgroupAddressSpace()));

  memref::AllocOp smemBuffer =
      builder.create<memref::AllocOp>(forallOp.getLoc(), smemBufferType);
...
LogicalResult copyWorkgroupMemoryToGlobalMemory(OpBuilder &b, Value src,
  ...
  if (OPERAND == MatmulOperand_A || OPERAND == MatmulOperand_B) {
    return success();
  }
  auto op = src.getDefiningOp();

  // Because MatmulC Operand is out of scf::ForOp so we need to get ForallOp
  // first
...
static LogicalResult promotionImpl(OpBuilder &builder, Operation *op) {
  ...
  std::optional<linalg::LinalgOp> promotedLinalgOp = linalg::promoteSubViews(
      builder, cast<linalg::LinalgOp>(op), promotionOptions);
