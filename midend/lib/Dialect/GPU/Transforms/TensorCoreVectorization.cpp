...
static void vectorizeLinalgOps(scf::ForallOp forallOp) {
    ...
    forallOp->walk([&](Operation *op){
        if (isa<linalg::FillOp, linalg::GenericOp, linalg::ContractionOpInterface>(op) &&
...
getReadVectorInContractOperandId(Operation *op) {
    if (op->use_empty()) {
        return std::nullopt;
    }
    ...
    if (isa<vector::ContractionOp>(firstLevelUser)) {
        vector::ContractionOp contractOp = dyn_cast<vector::ContractionOp>(firstLevelUser);
    ...
    if (isa<vector::ContractionOp>(secondLevelUser)) {
        vector::ContractionOp contractOp = dyn_cast<vector::ContractionOp>(secondLevelUser);
...
getTensorCoreNativeVectorShape(Operation *op) {
    ...
    if (isa<vector::TransferReadOp>(op)) {
        ...
        std::optional<int64_t> optionalOperandId = getReadVectorInContractOperandId(op);
...
public:
    ...
    void runOnOperation() override {
        ...
        // Use the following three steps to apply Vectorization (from Linalg to Vector)
        // Use vector.transfer_read / write to load / store the warp-level tile 
        // Use vector.contract to express calculation 
        vectorizeLinalgOps(forallOp);

        RewritePatternSet vectorizePatterns(ctx);
        vector::populateVectorTransferPermutationMapLoweringPatterns(vectorizePatterns);
        vector::populateVectorReductionToContractPatterns(vectorizePatterns);
        if (failed(applyPatternsAndFoldGreedily(funcOp, std::move(vectorizePatterns)))) {
            return signalPassFailure();
        }

        // Prepare vector operations to be lowered to TensorCore operations
        RewritePatternSet vectorMatchTensorCorePatterns(ctx);
        mlir::vector::populateCastAwayVectorLeadingOneDimPatterns(vectorMatchTensorCorePatterns);
        mlir::populatePrepareVectorToMMAPatterns(vectorMatchTensorCorePatterns, true);
        if (failed(applyPatternsAndFoldGreedily(funcOp, std::move(vectorMatchTensorCorePatterns)))) {
            return signalPassFailure();
        }
        ...
        vector::populateVectorUnrollPatterns(vectorUnrollPatterns, unrollVectorOptions);
        if (failed(applyPatternsAndFoldGreedily(funcOp, std::move(vectorUnrollPatterns)))) {
            return signalPassFailure();
        }
    }
};
