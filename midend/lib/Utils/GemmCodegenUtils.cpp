...
std::optional<SmallVector<int64_t, 3>> getGemmTileSize(func::FuncOp funcOp) {
  if (funcOp->hasAttr(getGemmTileMConfigAttrName()) &&
    ...
    auto tileConfigM =
        funcOp->getAttrOfType<IntegerAttr>(getGemmTileMConfigAttrName())
            .getInt();
    auto tileConfigN =
        funcOp->getAttrOfType<IntegerAttr>(getGemmTileNConfigAttrName())
            .getInt();
    auto tileConfigK =
        funcOp->getAttrOfType<IntegerAttr>(getGemmTileKConfigAttrName())
            .getInt();

    llvm::SmallVector<int64_t, 3> configVec = {tileConfigM, tileConfigN,
                                               tileConfigK};
...
std::optional<SmallVector<int64_t, 3>> getGemmBlockSize(func::FuncOp funcOp) {
  if (funcOp->hasAttr(getGemmBlockXSizeAttrName()) &&
    ...
    auto blockSizeX =
        funcOp->getAttrOfType<IntegerAttr>(getGemmBlockXSizeAttrName())
            .getInt();
    auto blockSizeY =
        funcOp->getAttrOfType<IntegerAttr>(getGemmBlockYSizeAttrName())
            .getInt();
    auto blockSizeZ =
        funcOp->getAttrOfType<IntegerAttr>(getGemmBlockZSizeAttrName())
            .getInt();

    llvm::SmallVector<int64_t, 3> blockSizeVec = {blockSizeX, blockSizeY,
                                                  blockSizeZ};
...
std::optional<int64_t> getGemmPipelineStages(func::FuncOp funcOp) {
  if (funcOp->hasAttr(getGemmPipelineStageAttrName())) {
    auto stages =
        funcOp->getAttrOfType<IntegerAttr>(getGemmPipelineStageAttrName())
            .getInt();
    return stages;
  }
  return std::nullopt;
