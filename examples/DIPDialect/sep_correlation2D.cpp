//===- sep_correlation2D.cpp - Example of buddy-opt tool ----------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements a 2D sep_correlation example with dip.corr_2d operation.
...
bool testImplementation(int argc, char *argv[], std::ptrdiff_t x,
  ...
  // Define the kernel.
  float *kernelAlignX = sepKernelX;
  int kernelRowsX = sepKernelXRows;
  int kernelColsX = sepKernelXCols;

  float *kernelAlignY = sepKernelY;
  int kernelRowsY = sepKernelYRows;
  int kernelColsY = sepKernelYCols;

  // Define sizes and strides.
  intptr_t sizesKernelX[2] = {kernelRowsX, kernelColsX};
  intptr_t sizesKernelY[2] = {kernelRowsY, kernelColsY};
  intptr_t sizesOutput[2] = {image.rows, image.cols};

  // Define memref containers.
  Img<float, 2> input(image);
  MemRef<float, 2> kernelX(kernelAlignX, sizesKernelX);
  MemRef<float, 2> kernelY(kernelAlignY, sizesKernelY);
  MemRef<float, 2> output1(sizesOutput);
  MemRef<float, 2> output2(sizesOutput);

  Mat kernel1X = Mat(1, 3 ,CV_32FC1, sepKernelX);
  Mat kernel1Y = Mat(3, 1, CV_32FC1, sepKernelY);

  // Call the MLIR Corr2D function.
  dip::Sep_Corr2D(&input, &kernelX, &kernelY, &output1, 0, 0,
  ...
  sepFilter2D(image, opencvReplicatePadding, CV_8UC1, kernel1X, kernel1Y, cv::Point(0, 0),
