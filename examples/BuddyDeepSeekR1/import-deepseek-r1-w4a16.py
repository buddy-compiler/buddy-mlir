...
# ===---------------------------------------------------------------------------
#
# W4A16: Int4 weight quantization, F16 activations for DeepSeekR1.
# Weights quantized to int4 range (-8..7) stored as int8, f16 compute.
#
# ===---------------------------------------------------------------------------
...
# Int4 quantization: scale = amax/7, weight in [-8, 7]
INT4_MAX = 7.0
...
for i, param_node in enumerate(graph_prefill.params):
    if i < num_original_params:
        ...
        if param_node.tensor_meta.get("dtype") == TensorDType.Int8:
            ...
            scale = (amax / INT4_MAX).clamp(min=1e-10)
            weight_i4 = torch.clamp(
                torch.round(original_tensor / scale), -8, 7
            ).to(torch.int8)

            all_param_tensors.append(("i8", weight_i4, param_node.name))
        else:
...
for dtype_tag, tensor, name in all_param_tensors:
    if dtype_tag == "f16":
        ...
        i4packed_params_data.append(packed)
...
with open(os.path.join(output_dir, "subgraph0_prefill-w4a16.mlir"), "w") as f:
    print(driver_prefill.subgraphs[0]._imported_module, file=f)
with open(os.path.join(output_dir, "forward_prefill-w4a16.mlir"), "w") as f:
    print(driver_prefill.construct_main_graph(True), file=f)

with open(os.path.join(output_dir, "subgraph0_decode-w4a16.mlir"), "w") as f:
    print(driver_decode.subgraphs[0]._imported_module, file=f)
with open(os.path.join(output_dir, "forward_decode-w4a16.mlir"), "w") as f:
    print(driver_decode.construct_main_graph(True), file=f)

if f16_params_data:
    all_f16 = numpy.concatenate(f16_params_data)
    all_f16.tofile(os.path.join(output_dir, "arg0-w4a16-f16.data"))

if i4packed_params_data:
    all_packed = numpy.concatenate(i4packed_params_data)
    all_packed.tofile(os.path.join(output_dir, "arg0-w4a16-i4packed.data"))
