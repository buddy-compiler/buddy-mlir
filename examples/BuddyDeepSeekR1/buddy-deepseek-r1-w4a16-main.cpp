//===- buddy-deepseek-r1-w4a16-main.cpp -----------------------------------===//
//
// W4A16 quantization: int4 weights with f16 activations for DeepSeekR1.
//
...
constexpr size_t I8ParamsSize = 727744512;
...
extern "C" void _mlir_ciface_forward_decode(
    MemRefContainer *result, MemRef<uint16_t, 1> *arg0, MemRef<int8_t, 1> *arg1,
...
int main() {
  ...
  const std::string title =
      "DeepSeekR1 W4A16 Quantized Inference Powered by Buddy Compiler";
  ...
  const std::string f16ParamsDir = deepSeekR1BuildDir + "arg0-w4a16-f16.data";
  const std::string i8ParamsDir =
      deepSeekR1BuildDir + "arg0-w4a16-i4packed.data";
  ...
  _mlir_ciface_forward_prefill(ptrPrefillResultContainer, &F16ParamsContainer,
  ...
  for (int i = 1; i <= generateLen; i++) {
    ...
    _mlir_ciface_forward_decode(
        ptrDecodeResultContainer, &F16ParamsContainer, &I8ParamsContainer,
