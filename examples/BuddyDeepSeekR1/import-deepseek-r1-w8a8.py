...
# ===---------------------------------------------------------------------------
#
# W8A8 quantized DeepSeekR1 model importer.
# Converts the graph to use native int8 matmul with dynamic activation
# quantization.
#
# ===---------------------------------------------------------------------------
...
parser = argparse.ArgumentParser(
    description="DeepSeekR1 W8A8 Quantized AOT Importer"
)
...
# Phase 2: Apply graph transformations and W8A8 quantization
...
# Phase 3: Compute actual int8 weights and scale values
all_param_tensors = []
...
# Save MLIR files
with open(os.path.join(output_dir, "subgraph0_prefill-w8a8.mlir"), "w") as f:
    print(driver_prefill.subgraphs[0]._imported_module, file=f)
with open(os.path.join(output_dir, "forward_prefill-w8a8.mlir"), "w") as f:
    print(driver_prefill.construct_main_graph(True), file=f)

with open(os.path.join(output_dir, "subgraph0_decode-w8a8.mlir"), "w") as f:
    print(driver_decode.subgraphs[0]._imported_module, file=f)
with open(os.path.join(output_dir, "forward_decode-w8a8.mlir"), "w") as f:
    print(driver_decode.construct_main_graph(True), file=f)

# Save param data files
if f32_params_data:
    all_f32 = numpy.concatenate(f32_params_data)
    all_f32.tofile(os.path.join(output_dir, "arg0-w8a8-f32.data"))

if i8_params_data:
    all_i8 = numpy.concatenate(i8_params_data)
    all_i8.tofile(os.path.join(output_dir, "arg0-w8a8-i8.data"))
