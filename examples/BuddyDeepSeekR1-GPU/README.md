...
This example shows how to use Buddy Compiler to compile a DeepSeek-R1-Distill-Qwen-1.5B model to MLIR code then run it on NVIDIA GPUs.

The model is compiled through MLIR's GPU pipeline (linalg → parallel-loops → GPU blocks → CUDA kernels via NVVM) with CUDA managed memory for seamless CPU/GPU data sharing.
...
```
Alternatively, you can leave the path blank, and import-deepseek-r1-gpu.py will automatically download the model for you.

4. Build and run the DeepSeekR1 GPU example

```bash
$ cmake -G Ninja .. -DBUDDY_DEEPSEEKR1_EXAMPLES=ON
$ ninja buddy-deepseek-r1-gpu-run
$ CUDA_VISIBLE_DEVICES=0 ./bin/buddy-deepseek-r1-gpu-run
```
