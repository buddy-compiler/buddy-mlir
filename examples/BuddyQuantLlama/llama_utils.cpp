...
void loadQuantizedModelConfig(const std::string &configFilePath, QuantizedModelConfig &config) {
  std::cout << "Config file: " << std::filesystem::canonical(configFilePath)
            << std::endl;
  std::ifstream configFile(configFilePath, std::ios::in);
  std::string s;
  configFile >> s >> config.float32ParamSize;
  assert(s == "float32ParamSize");
  std::cout << s << " = " << config.float32ParamSize << std::endl;
  configFile >> s >> config.int64ParamSize;
  assert(s == "int64ParamSize");
  std::cout << s << " = " << config.int64ParamSize << std::endl;
  configFile >> s >> config.int8ParamSize;
  assert(s == "int8ParamSize");
  std::cout << s << " = " << config.int8ParamSize << std::endl;
  configFile >> s >> config.hiddenSize;
  assert(s == "hiddenSize");
  std::cout << s << " = " << config.hiddenSize << std::endl;
  configFile >> s >> config.maxVocabSize;
