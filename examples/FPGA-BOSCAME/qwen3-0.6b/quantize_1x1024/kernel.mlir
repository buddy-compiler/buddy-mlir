module {
func.func @kernel_quantize_1x1024(%x: memref<1x1024xf32>, %q: memref<1x1024xi8>, %scale: memref<1xf32>) attributes {llvm.emit_c_interface} {
 ...
 linalg.generic {indexing_maps=[affine_map<(i,j)->(i,j)>,affine_map<(i,j)->(i)>], iterator_types=["parallel","reduction"]} ins(%x:memref<1x1024xf32>) outs(%scale:memref<1xf32>) {
 ^bb0(%v:f32,%a:f32):
  %abs = math.absf %v : f32
 ...
 linalg.generic {indexing_maps=[affine_map<(i,j)->(i,j)>,affine_map<(i,j)->(i)>,affine_map<(i,j)->(i,j)>],iterator_types=["parallel","parallel"]} ins(%x,%scale:memref<1x1024xf32>,memref<1xf32>) outs(%q:memref<1x1024xi8>) {
