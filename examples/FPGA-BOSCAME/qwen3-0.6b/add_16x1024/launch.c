...
int launch(void) {
  ...
  for (int i=0; i<16384; ++i) { x[i] = (float)((i*13)%129-64)*0.125f; out[i] = 9999.0f; y[i] = (float)((i*7)%31-15)*0.0625f; }
  _mlir_ciface_kernel_add_16x1024(&m_x, &m_y, &m_out);
  for (int i=0; i<16384; ++i) compare(out[i], x[i] + y[i], 2e-6f, 3e-5f, &errors, &maximum);
