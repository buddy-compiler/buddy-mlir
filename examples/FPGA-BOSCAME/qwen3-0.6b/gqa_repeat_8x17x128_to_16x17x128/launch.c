...
extern void _mlir_ciface_kernel_gqa_repeat_8x17x128_to_16x17x128(MemRef3 *, MemRef3 *);
int launch(void) {
  ...
  for(int i=0;i<17408;++i) x[i]=(float)((i*11)%127-63)*0.03125f;
  for(int i=0;i<34816;++i) out[i]=9999;
  _mlir_ciface_kernel_gqa_repeat_8x17x128_to_16x17x128(&m_x, &m_out);
  for(int h=0;h<16;++h) for(int t=0;t<17;++t) for(int d=0;d<128;++d) {
    compare(out[(h*17+t)*128+d],x[((h/2)*17+t)*128+d],0,0,&errors,&maximum);
  }
