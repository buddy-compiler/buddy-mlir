...
def boundary_sync_identity(image, plan, build_record):
    manifest_path, source_path = image/'boundary-sync.json', image/'boundary-sync.c'
    bound = plan.get('boundary_sync')
    if bound is None:
        require(not any(path.exists() or path.is_symlink() for path in
                        (manifest_path, source_path, image/'boundary-sync-selection.json'))
                and not plan.get('boundary_sync_requested'),
                'boundary sync evidence absent from exact image plan')
        return None
    manifest = read(safe_file(image, 'boundary-sync.json'))
    require(bound == manifest and manifest.get('schema_version') == 1,
            'boundary sync plan/manifest mismatch')
    source_path = safe_file(image, 'boundary-sync.c')
    find_input(build_record, source_path)
    require(digest(source_path) == manifest.get('generated_source_sha256'),
            'boundary sync generated source mismatch')
    symbols = manifest.get('symbols')
    require(isinstance(symbols, list) and symbols
            and all(isinstance(s, str) and re.fullmatch(r'_mlir_ciface_qwen_graph_\w+', s) for s in symbols)
            and symbols == sorted(set(symbols)),
            'invalid boundary sync symbols')
    require(manifest.get('linker_flags') == ['--wrap=' + symbol for symbol in symbols]
            and manifest.get('completion_sync') == 'ame-resync', 'boundary sync contract mismatch')
    dependencies = manifest.get('source_sha256')
    require(isinstance(dependencies, dict) and dependencies, 'missing boundary sync source hashes')
    for name, expected in dependencies.items():
        path = Path(name) if Path(name).is_absolute() else REPO/name
        require(not path.is_symlink() and path.is_file() and digest(path) == expected,
                'boundary sync source hash mismatch: ' + name)

    # The copied selection must be the exact manifest consumed by the build,
    # not merely another schema-valid selection next to the same ELF.
    request = plan.get('boundary_sync_requested')
    require(isinstance(request, dict) and isinstance(request.get('manifest'), str)
            and request['manifest'], 'missing boundary sync selection request')
    prior_path = Path(request['manifest'])
    if not prior_path.is_absolute():
        prior_path = REPO/prior_path
    require(prior_path.is_file() and not prior_path.is_symlink(), 'missing boundary sync prior selection')
    find_input(build_record, prior_path)
    find_input({'input_sha256': dependencies}, prior_path)
    selection_path = safe_file(image, 'boundary-sync-selection.json')
    selection_hash = digest(selection_path)
    require(selection_hash == request.get('manifest_sha256') == digest(prior_path),
            'boundary sync copied selection hash mismatch')
    from check_intermediates import validate_manifest
    from intermediate_probe import tensor_types
    selection = read(selection_path)
    validate_manifest(selection)
    require(selection['schema_version'] == 2, 'boundary sync selection requires schema 2')
    require(manifest.get('selected_layers') == selection['selected_layers'],
            'boundary sync selected layers differ from copied selection')
    require(symbols == sorted({entry['symbol'] for entry in selection['entries']}),
            'boundary sync symbols differ from full copied selection')
    for field in ('layers', 'prefill_len', 'decode_steps'):
        require(manifest.get(field) == selection[field], 'boundary sync selection dimensions mismatch')

    declarations = {}
    for entry in selection['entries']:
        line = entry.get('ir_line')
        call = re.search(r'\bcall @(qwen_graph_\w+)\(([^)]*)\)\s*:\s*\((.*?)\)\s*->\s*(.*)$',
                         line) if isinstance(line, str) else None
        require(call is not None and '_mlir_ciface_' + call[1] == entry['symbol'],
                'boundary sync selection lacks matching typed call')
        inputs, outputs = tensor_types(call[3]), tensor_types(call[4])
        require(len(inputs) == len(call[2].split(',')) and len(outputs) <= 1,
                'unsupported boundary sync selection call ABI')
        arguments = outputs + inputs
        ranks = [len(argument['shape']) for argument in arguments]
        require(all(1 <= rank <= 4 for rank in ranks) and entry['operand'] < len(arguments)
                and arguments[entry['operand']] == {'shape': entry['shape'], 'dtype': entry['dtype']},
                'boundary sync selection operand ABI mismatch')
        require(entry['symbol'] not in declarations or declarations[entry['symbol']] == ranks,
                'conflicting boundary sync selection call ABI')
        declarations[entry['symbol']] = ranks
    expected_source = '#include "support.h"\n#include "nr_runtime.h"\n'
    for symbol in symbols:
        ...
        expected_source += (f'extern void __real_{symbol}({params});\n'
                            f'void __wrap_{symbol}({params}) {{\n'
                            f'  __real_{symbol}({operands});\n  ame_fence();\n}}\n')
    require(re.sub(r'\s+', '', source_path.read_text()) == re.sub(r'\s+', '', expected_source),
            'boundary sync source differs from selected identity-forwarding call/fence wrappers')

    linker = build_record.get('linker', {}).get('path')
    commands = build_record.get('commands', [])
    require(isinstance(linker, str) and linker and isinstance(commands, list),
            'missing boundary sync linker evidence')
    links = [command for command in commands if isinstance(command, list) and command
             and command[0] == linker]
    require(len(links) == 1 and all(isinstance(arg, str) for arg in links[0]),
            'missing/ambiguous boundary sync link command')
    wraps = [arg for arg in links[0] if arg.lstrip('-').startswith('wrap')]
    require(wraps == manifest['linker_flags'], 'boundary sync actual link wrap flags mismatch')
    require(manifest['layers'] == plan['layers'], 'boundary sync image layer count mismatch')
    for field in ('prefill_len', 'decode_steps'):
        require(manifest[field] == plan['numeric_reference'][field],
                'boundary sync image dimensions mismatch')
    return {'status': 'BUILD_SOURCE_AND_PLAN_VERIFIED', 'symbols': symbols,
            'manifest_sha256': digest(manifest_path), 'source_sha256': digest(source_path),
            'selection_sha256': selection_hash, 'selected_layers': selection['selected_layers'],
            'linker_flags': wraps,
        ...
        for entry in manifest['entries']:
            ...
            for key, offset in zip(entry['reference_keys'], entry['offsets']):
                ...
                for alias in entry.get('reference_aliases', []):
                    ...
                    if manifest.get('coverage_includes_aliases', False):
                        covered.add(alias_key)
...
def archive(args):
    ...
    boundary_identity = boundary_sync_identity(image, plan, build_record)
    ...
    if boundary_identity is not None:
        require(all('__wrap_' + name in symbols for name in boundary_identity['symbols']),
                'boundary sync wrapper missing from ELF')
    ...
    with tempfile.TemporaryDirectory(prefix='.model-archive-', dir=args.output.parent) as temp:
        ...
        if boundary_identity is not None:
            validator_files += ['check_intermediates.py', 'intermediate_probe.py']
        ...
        report = {'status': 'MODEL_RUN_NUMERIC_PASS', 'run_id': run.name, 'fpga': result['fpga'],
            ...
            'identity': {'elf_sha256': digest(elf), 'boot_matches_elf_objcopy': True,
                         ...
                         'boundary_sync': boundary_identity,
