#!/usr/bin/env python3
"""Check the complete 1-layer, 16-token prefill + 8-decode intermediate trace.
...
def validate_manifest(manifest):
    ...
    for name, wanted in [('schema_version', 1), ('layers', 1), ('prefill_len', 16), ('decode_steps', 8)]:
    ...
    require(isinstance(entries, list) and len(entries) == 92, 'expected exactly 92 intermediate entries')
    ...
    for index, entry in enumerate(entries):
        ...
        require(entry.get('graph') == graph and type(entry.get('layer')) is int and entry['layer'] == 0,
    ...
    return {'prefill': entries[:46], 'decode': entries[46:]}
...
def check(log, manifest):
    ...
    for number, line in enumerate(log.splitlines(), 1):
        ...
        if match:
            ...
            entry = manifest['entries'][index] if index < 92 else None
    ...
    if len(records) != 414:
        errors.append(f'expected exactly 414 intermediate comparisons, found {len(records)}')
    ...
    return {'status': 'INTERMEDIATES_PASS' if not errors else 'NOT_ACCEPTED',
            'scope': SCOPE, 'errors': errors, 'comparisons': len(records), 'expected_comparisons': 414,
            'entries_per_graph': {'prefill': 46, 'decode': 46}, 'graph_positions': POSITIONS,
...
def main():
    ...
    try:
        ...
        report = {'status': 'NOT_ACCEPTED', 'scope': SCOPE, 'errors': [str(error)]}
