...
def check(log, expected, *, prefill=16, steps=8):
    ...
    return {'status': 'KERNEL_PROFILE_PASS' if not errors else 'NOT_ACCEPTED',
            ...
            'limits': ['Timing/count validation only; numerical acceptance requires check_board_trace.py.',
                       'Kernel timing includes the added AME completion fence; instrumentation changes execution cost.',
...
def main():
    ...
    try:
        ...
        if progress:
            report['limits'].append('Per-kernel diagnostic UART is inside graph timing. Kernel cycles exclude its writes; graph/model cycles are not UART-free throughput.')
