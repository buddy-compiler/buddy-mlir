...
#define K 512
#define QK 0
extern void _mlir_ciface_kernel_attention_pv_position_16x1x128x512_bounds(MemRef3*,MemRef3*,MemRef1*,MemRef3*);
int launch(void) {
  ...
  for(unsigned step=0;step<sizeof(lengths)/sizeof(lengths[0]);++step) {
    ...
    _mlir_ciface_kernel_attention_pv_position_16x1x128x512_bounds(&A,&B,&P,&C);
  ...
  return print_check("attention_pv_position_16x1x128x512_bounds",errors,maximum);
}
