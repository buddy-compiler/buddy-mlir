 ...
 * it finds here, and RA consumes it with nr_getchar(). RA accesses shared DDR
 * without NH cache instructions. Each writer owns a separate cache line: NH
 * flushes head/data, and invalidates the RA-owned tail before reading it. */
...
static uint32_t drain(uint32_t consumed) {
  ...
  if ((uint32_t)(end - consumed) > 256u) end = consumed + 256u;
  while (consumed != end) {
    uint32_t slot = consumed & (LOG_CAPACITY - 1u);
    invalidate(&console.data[slot]);
    fence();
    uint32_t amount = 64u - (slot & 63u);
    if (amount > end - consumed) amount = end - consumed;
    while (amount--) { uart_putc(console.data[slot++]); ++consumed; }
...
__attribute__((noreturn)) void nr_nh_main(void) {
  ...
  *RA_SIGNAL = 0;
  ...
  do { consumed = drain(consumed); } while (consumed != console.count);
  host_puts(result == 1 ? "[nr] RA returned: PASS\r\nverify NR runtime: PASS\r\n" :
