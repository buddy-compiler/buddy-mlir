...
#define K 128
#define QK 1
extern void _mlir_ciface_kernel_attention_qk_position_16x16x512x128_bounds(MemRef3*,MemRef3*,MemRef1*,MemRef3*);
int launch(void) {
  ...
  for(unsigned step=0;step<sizeof(lengths)/sizeof(lengths[0]);++step) {
    ...
    _mlir_ciface_kernel_attention_qk_position_16x16x512x128_bounds(&A,&B,&P,&C);
  ...
  return print_check("attention_qk_position_16x16x512x128_bounds",errors,maximum);
}
