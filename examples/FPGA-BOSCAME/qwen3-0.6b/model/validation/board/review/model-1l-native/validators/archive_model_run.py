...
def archive(args):
    require(not args.output.exists(), 'refusing to overwrite existing archive: ' + str(args.output))
    ...
    with tempfile.TemporaryDirectory(prefix='.model-archive-', dir=args.output.parent) as temp:
        ...
        for folder in ('replacement','nr-prefill','nr-decode','model-lib'):
            for source in sorted((build/folder).rglob('*')):
                if source.is_file() and source.suffix in ('.c','.h','.ll','.mlir','.json','.txt','.map','.s','.o','.a'):
        ...
        for filename in ('archive_model_run.py','check_board_trace.py','check_kernel_profile.py'):
        ...
        require(not args.output.exists(), 'archive destination appeared during validation')
