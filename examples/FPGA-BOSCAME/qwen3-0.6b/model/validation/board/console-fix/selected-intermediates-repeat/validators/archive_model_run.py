...
def boundary_sync_identity(image, plan, build_record):
    ...
    if bound is None:
        require(not manifest_path.exists() and not source_path.exists(),
    ...
    require(isinstance(symbols, list) and symbols and symbols == sorted(set(symbols))
            and all(isinstance(s, str) and s.startswith('_mlir_ciface_qwen_graph_') for s in symbols),
