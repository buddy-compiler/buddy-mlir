...
Physical cache strides use --capacity (128 by default). Each executable grows
and shrinks the valid length, poisons invalid inputs with NaNs, and checks every
output. Binary-exact fraction inputs make the independent double oracle exact
for either fused or unfused FP32 dot products. No model deployment uses these
cases implicitly.
"""
import argparse
import json
...
def valid_lengths(sequence, boundary_probe=False, native_key=False, capacity=128):
    if capacity < max(sequence, 24):
        raise ValueError('capacity must hold the sequence and 16+8 validation window')
    if boundary_probe:
    ...
    if native_key:
        ...
        lengths = [length for length in lengths if sequence <= length <= capacity]
    else:
        lengths = [length for length in [16,17,24,63,64,65,511,512,511,65,64,63,24,17,16]
                   if sequence <= length <= capacity]
    if capacity not in lengths:
        lengths.extend([capacity - 1, capacity, sequence])
    return lengths


def launcher(name, sequence, qk, boundary_probe=False, native_key=False, capacity=128):
    n, k = (capacity, 128) if qk else (128, capacity)
    lengths = ",".join(map(str, valid_lengths(sequence, boundary_probe, native_key, capacity)))
...
def main():
    ...
    for sequence in (1,16):
        for qk in (True,False):
            ...
            n,k=(args.capacity,128) if qk else (128,args.capacity)
            ...
            meta=attention_metadata(name,family,(16,sequence,n,k),
                f"Runtime valid length; physical capacity {args.capacity}; NaN tail validation")
            meta.update(runtime_position=True,valid_lengths=valid_lengths(sequence,args.boundary_probe,args.native_key,args.capacity),
            ...
            (directory/"launch.c").write_text(launcher(name,sequence,qk,args.boundary_probe,args.native_key,args.capacity))
            ...
            names.append(name)
    (args.output/"cases.json").write_text(json.dumps({"capacity":args.capacity,"cases":names},indent=2)+"\n")
    print("\n".join(names))
