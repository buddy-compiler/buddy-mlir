...
extern void _mlir_ciface_kernel_softmax_16x1x17(MemRef3 *, MemRef2 *, MemRef2 *, MemRef3 *);
int launch(void) {
  ...
  for (int row=0; row<16; ++row) for (int k=0; k<17; ++k) {
    int i=row*17+k; x[i]=80.0f+(float)((i*7)%53-26)*0.25f; out[i]=9999;
    if (k==5) x[i]=-__builtin_inff();
  }
  _mlir_ciface_kernel_softmax_16x1x17(&m_x, &m_maxima, &m_sums, &m_out);
  for (int row=0; row<16; ++row) {
    float maximum_input=-__builtin_inff(); oracle_float sum=0;
    for (int k=0; k<17; ++k) if(x[row*17+k]>maximum_input) maximum_input=x[row*17+k];
    for (int k=0; k<17; ++k) sum+=oracle_exp(x[row*17+k]-maximum_input);
    oracle_float output_sum=0;
    for (int k=0; k<17; ++k) { int i=row*17+k; float expected=(float)(oracle_exp(x[i]-maximum_input)/sum); compare(out[i],expected,2e-6f,5e-5f,&errors,&maximum); output_sum+=out[i]; }
