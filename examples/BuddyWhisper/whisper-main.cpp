...
extern "C" void _mlir_ciface_forward(MemRef<float, 3> *, MemRef<float, 1> *, MemRef<float, 3> *,
                                     MemRef<size_t, 2> *);
...
void loadParameters(const std::string &paramFilePath, MemRef<float, 1> &params) {
  ...
  printLogLabel();
  std::cout << "Params file: " << std::filesystem::canonical(paramFilePath) << std::endl;
  paramFile.read(reinterpret_cast<char *>(params.getData()), sizeof(float) * (params.getSize()));
  if (paramFile.fail()) {
  ...
  printLogLabel();
  std::cout << "Params load time: " << (double)(loadTime.count()) / 1000 << "s\n" << std::endl;
}
...
void loadAudio(const std::string &paramFilePath, MemRef<float, 3> &params) {
  ...
  printLogLabel();
  std::cout << "input_features file: " << std::filesystem::canonical(paramFilePath) << std::endl;
  ...
  const std::chrono::duration<double, std::milli> loadTime = loadEnd - loadStart;
  printLogLabel();
  std::cout << "input_features load time: " << (double)(loadTime.count()) / 1000 << "s\n"
            << std::endl;
}
...
int main() {
  ...
  const std::string input_featuresDir = "../../examples/BuddyWhisper/input_features.data";
  ...
  for (int i = 0; i < MaxTokenLength - 1; i++) {
    ...
    _mlir_ciface_forward(resultContainer, &paramsContainer, &audioInput, &textContainer);
    ...
    std::cout << outputContainer.getData()[i] << std::endl;
  ...
  std::cout << "\033[33;1m[Output]\033[0m " << outputContainer.revertWhisper() << std::endl;
