// RUN: buddy-opt %s --lower-xtame | FileCheck %s
...
module {
  ...
  func.func @main() -> i32 {
    ...
    %c_ptr = memref.alloc() : memref<4x4xi32>    // 结果矩阵 C
    %a_ptr = memref.alloc() : memref<4x4xi8>     // 矩阵 A
    %b_ptr = memref.alloc() : memref<4x4xi8>     // 矩阵 B

    %stride_a = arith.constant 4 : i64               // A 的行跨度 (4 bytes per row)
    %stride_b = arith.constant 4 : i64               // B 的行跨度 (4 bytes per row)
    %stride_c = arith.constant 16 : i64              // C 的行跨度 (16 bytes per row, 因为每个元素是 4 bytes)
    ...
    xtame.th.mcfgmi 4          // mtilem = 4 (rows of C and A)
    xtame.th.mcfgni 4          // mtilen = 4 (cols of C and B)
    xtame.th.mcfgki 4          // mtilek = 4 (cols of A, rows of B)
    
    // Step 2: Zero the accumulation register (acc register 0)
    xtame.th.mzero 0
    
    // Step 3: Load matrix A to tile register 0 (shape: mtilem x mtilek = 4x4)
    xtame.th.mlde8 1, %stride_a, %a_ptr: memref<4x4xi8>
    
    // Step 4: Load matrix B to tile register 1 (shape: mtilek x mtilen = 4x4)
    xtame.th.mldte8 2, %stride_b, %b_ptr: memref<4x4xi8>
    
    // Step 5: Execute matrix multiply: acc0 = acc0 + tile0 x tile1
    xtame.th.mmacc.w.b 0, 2, 1
    
    xtame.th.mcfgki 16         // mtilek = 16 
    // Step 6: Store result from accumulator 0 to memory
    xtame.th.mste32 0, %stride_c, %c_ptr: memref<4x4xi32>
