module attributes { transform.with_named_sequence } {
  transform.named_sequence @codegen(%arg0: !transform.any_op) {
    ...
    %matmul = transform.structured.match ops{["linalg.batch_matmul"]} in %arg0
    ...
    %tiled_op, %forall_op = transform.structured.tile_using_forall %matmul
      tile_sizes [2,128, 256] (mapping = [#gpu.block<z>, #gpu.block<y>, #gpu.block<x>])
    ...
    %padded, %pad, %copy = transform.structured.pad %tiled_linalg_op {copy_back_op = "none", pack_paddings = [1, 1, 1, 1], pad_to_multiple_of = [1, 1, 1, 1], padding_dimensions = [0, 1, 2, 3], padding_values = [0.000000e+00 : f32, 0.000000e+00 : f32, 0.000000e+00 : f32, 0.000000e+00 : f32]} : (!transform.any_op) -> (!transform.any_op, !transform.any_op, !transform.any_op)
    ...
    %tiled_op_2, %forall_op_3 = transform.structured.tile_using_forall %7 num_threads [1,8, 16](mapping = [#gpu.thread<linear_dim_2>,#gpu.thread<linear_dim_1>, #gpu.thread<linear_dim_0>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    %tiled_op_0, %forall_op_1 = transform.structured.tile_using_forall %6 num_threads [1,32, 4](mapping = [#gpu.thread<linear_dim_2>,#gpu.thread<linear_dim_1>, #gpu.thread<linear_dim_0>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    

    // Tile the linalg.matmul op and map it to GPU warp level.
    %tiled_op_4, %forall_op_5 = transform.structured.tile_using_forall %padded num_threads [1,2, 2](mapping = [#gpu.warp<z>,#gpu.warp<y>, #gpu.warp<x>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    // Tile the linalg.fill op and map it to GPU warp level.
    %tiled_op_6, %forall_op_7 = transform.structured.tile_using_forall %fused_op num_threads [1,2, 2](mapping = [#gpu.warp<z>, #gpu.warp<y>, #gpu.warp<x>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    ...
    %scf_forall = transform.structured.match ops{["scf.forall"]} attributes{mapping = [#gpu.block<z>, #gpu.block<y>, #gpu.block<x>]} in %arg0 : (!transform.any_op) -> !transform.any_op
