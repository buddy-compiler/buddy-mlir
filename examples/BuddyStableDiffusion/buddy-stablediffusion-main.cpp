...
// SchedulerConfig structure, which contains the necessary configuration information
struct SchedulerConfig {
    ...
    SchedulerConfig(const MemRef<float, 1>& alphas, const MemRef<float, 4>& sample)
        : alphas_cumprod(alphas), cur_sample(sample) {}
};

MemRef<float, 1> generate_betas(float beta_start, float beta_end, size_t num_train_timesteps) {
    ...
    for (size_t i = 0; i < num_train_timesteps; i++) {
        float t = static_cast<float>(i) / (num_train_timesteps - 1); // Calculate the scale
        float value = start_sqrt + t * (end_sqrt - start_sqrt); // Linear interpolation
...
MemRef<float, 4> memref_mul_scalar(const MemRef<float, 4>& memref, float scalar) {
...
MemRef<float, 4> memref_add(const MemRef<float, 4>& a, const MemRef<float, 4>& b) {
    MemRef<float, 4> result({1, 4, 64, 64});
    for (int i = 0; i < 1 * 4 * 64 * 64; ++i) {
        result[i] = a[i] + b[i];
    }
...
MemRef<float, 4> get_prev_sample(
    ...
    float alpha_prod_t_prev = (prev_timestep >= 0) ? config.alphas_cumprod.getData()[prev_timestep] : config.final_alpha_cumprod;
    ...
    if (config.prediction_type == "v_prediction") {
        ...
        for (int i = 0; i < 1 * 4 * 64 * 64; ++i) {
            prev_sample[i] = 
                std::sqrt(alpha_prod_t) * model_output[i] + 
                std::sqrt(beta_prod_t) * sample[i];
        }
    } else if (config.prediction_type != "epsilon") {
        throw std::invalid_argument("prediction_type must be one of `epsilon` or `v_prediction`");
    }
    ...
    for (int i = 0; i < 1 * 4 * 64 * 64; ++i) {
        prev_sample[i] = 
            sample_coeff * sample[i] - 
            (alpha_prod_t_prev - alpha_prod_t) * model_output[i] / model_output_denom_coeff;
    }
...
MemRef<float, 4> step_plms(
    const MemRef<float, 4> model_output,
    int timestep,
    MemRef<float, 4> sample,
    ...
    if (counter != 1) {
        if (ets.size() > 3) ets.erase(ets.begin(), ets.begin() + ets.size() - 3);
    ...
    if (ets.size() == 1 && counter == 0) {
        ...
        updated_model_output = memref_mul_scalar(memref_add(model_output, ets.back()), 0.5);
        sample = config.cur_sample;
    } else if (ets.size() == 2) {
        updated_model_output = memref_mul_scalar(memref_add(memref_mul_scalar(ets.back(), 3.0), memref_mul_scalar(ets[ets.size() - 2], -1.0)), 0.5);
    } else if (ets.size() == 3) {
        updated_model_output = memref_mul_scalar(memref_add(memref_add(memref_mul_scalar(ets.back(), 23.0), memref_mul_scalar(ets[ets.size() - 2], -16.0)), memref_mul_scalar(ets[ets.size() - 3], 5.0)), 1.0 / 12.0);
    } else {
        updated_model_output = memref_mul_scalar(memref_add(memref_add(memref_add(memref_mul_scalar(ets.back(), 55.0), memref_mul_scalar(ets[ets.size() - 2], -59.0)), memref_mul_scalar(ets[ets.size() - 3], 37.0)), memref_mul_scalar(ets[ets.size() - 4], -9.0)), 1.0 / 24.0);
    }

    MemRef<float, 4> prev_sample = get_prev_sample(sample, timestep, prev_timestep, updated_model_output, config);
...
std::vector<int> set_timesteps(int num_inference_steps) {
  ...
  timesteps.insert(timesteps.end(), plms_timesteps.begin(), plms_timesteps.end());
  if (num_inference_steps == 1)
    timesteps.resize(1);
  return timesteps;
}
...
extern "C" void
_mlir_ciface_forward_text_encoder(MemRefContainer *result, MemRef<float, 1> *arg0, 
                                  MemRef<long long, 1> *arg1, MemRef<long long, 2> *arg2);

extern "C" void
_mlir_ciface_forward_unet(MemRef<float, 4> *result, 
                     MemRef<float, 1> *arg0, MemRef<float, 4> *arg1, 
                     MemRef<float, 1> *arg2, MemRef<float, 3> *arg3);

extern "C" void
_mlir_ciface_forward_vae(MemRef<float, 4> *result, 
                         MemRef<float, 1> *arg0,
                         MemRef<float, 4> *arg1);
...
int main() {
  ...
  _mlir_ciface_forward_text_encoder(ptrPos, &arg0_text_encoder, &arg1_text_encoder, &TextEncoderInputIDsPos);
  _mlir_ciface_forward_text_encoder(ptrNeg, &arg0_text_encoder, &arg1_text_encoder, &TextEncoderInputIDsNeg);
  ...
  for (int i = 0; i < 2 * 77 * 1024 ; i ++ ){
    if (i < 1 * 77 * 1024)
      ...
      TextEncoderOut.getData()[i] = TextEncoderOutPos.getData()[i % (1 * 77 * 1024)];
  }
  ...
  for (int i = 0 ; i < 1000 ; i ++ ){
    ...
    if (i >= 1)
      alphas_cumprod.getData()[i] = alphas_cumprod.getData()[i] * alphas_cumprod.getData()[i - 1];
  ...
  for (int i = 0; i < (int)timesteps.size() ; i ++){
    ...
    for (int j = 0 ; j < 1 * 4 * 64 * 64; j ++){
      pred_noise.getData()[j] = resultUnet.getData()[j] + 7.5 * (resultUnet.getData()[j + 1 * 4 * 64 * 64] - resultUnet.getData()[j]);
    }
