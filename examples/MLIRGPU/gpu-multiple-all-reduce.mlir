...
func.func @main() {
  ...
  // ADD + MUL
  // launche a GPU kernel using gpu.launch, The kernel uses the following steps:
  // * load a value from the memref %data using memref.load, which is a memory load operation. 
  //   The value is determined by the block index %bx and the thread index %tx, such that each 
  //   thread loads one element from the memref %data.
  // * perform an all-reduce operation on the loaded value using gpu.all_reduce, which is a collective
  //   communication operation. The operation uses the add operation as the reduction function, and 
  //   returns the sum of all the values in the same block. 
  //   The operation is uniform, meaning that all the threads in the same block participate in the operation 
  //   and receive the same result.
  // * store the result of the all-reduce operation to the memref %sum using memref.store, which is a memory 
  //   store operation. The result is stored at the index determined by the block index %bx, such that each 
  //   block stores one element to the memref %sum.
  // 
