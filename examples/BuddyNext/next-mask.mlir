...
func.func @kernel() {
  ...
  %9 = tosa.add %7, %8 : (tensor<40xi64>, tensor<40xi64>) -> tensor<40xi64>
    %shape_10 = tosa.const_shape {values = dense<[40, 1]> : tensor<2xindex>} : () -> !tosa.shape<2>
  %10 = tosa.reshape %9, %shape_10 : (tensor<40xi64>, !tosa.shape<2>) -> tensor<40x1xi64>
  ...
  %extracted_slice = tensor.extract_slice %cst[0, 0] [1, 40] [1, 1] : tensor<1x40xi1> to tensor<1x40xi1>
    %shape_15 = tosa.const_shape {values = dense<[1, 1, 40]> : tensor<3xindex>} : () -> !tosa.shape<3>
  %15 = tosa.reshape %extracted_slice, %shape_15 : (tensor<1x40xi1>, !tosa.shape<3>) -> tensor<1x1x40xi1>
    %shape_16 = tosa.const_shape {values = dense<[1, 1, 1, 40]> : tensor<4xindex>} : () -> !tosa.shape<4>
  %16 = tosa.reshape %15, %shape_16 : (tensor<1x1x40xi1>, !tosa.shape<4>) -> tensor<1x1x1x40xi1>
  ...
  ^bb0(%in: i1, %in_742: f32, %out: f32):
    ...
    %shape_25 = tosa.const_shape {values = dense<[1, 40, 40]> : tensor<3xindex>} : () -> !tosa.shape<3>
  %25 = tosa.reshape %14, %shape_25 : (tensor<40x40xf32>, !tosa.shape<3>) -> tensor<1x40x40xf32>
    %shape_26 = tosa.const_shape {values = dense<[1, 1, 40, 40]> : tensor<4xindex>} : () -> !tosa.shape<4>
  %26 = tosa.reshape %25, %shape_26 : (tensor<1x40x40xf32>, !tosa.shape<4>) -> tensor<1x1x40x40xf32>
