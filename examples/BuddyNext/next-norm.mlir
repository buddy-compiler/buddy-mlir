...
func.func @kernel(%t0: tensor<1x40x4096xf32>, %t1: tensor<4096xf32>) {
  ...
  %temp = tosa.reshape %34, %s0 : (tensor<1xf32>, !tosa.shape<3>) -> tensor<1x1x1xf32>
  %35 = tosa.mul %temp, %32, %shift : (tensor<1x1x1xf32>, tensor<1x40x1xf32>, tensor<1xi8>) -> tensor<1x40x1xf32>
  ...
  %39 = tosa.mul %t0, %38, %shift : (tensor<1x40x4096xf32>, tensor<1x40x1xf32>, tensor<1xi8>) -> tensor<1x40x4096xf32>
  %s1 = tosa.const_shape {values = dense<[1, 1, 4096]> : tensor<3xindex>} : () -> !tosa.shape<3>
  %40 = tosa.reshape %t1, %s1 : (tensor<4096xf32>, !tosa.shape<3>) -> tensor<1x1x4096xf32>
  %41 = tosa.mul %40, %39, %shift : (tensor<1x1x4096xf32>, tensor<1x40x4096xf32>, tensor<1xi8>) -> tensor<1x40x4096xf32>
