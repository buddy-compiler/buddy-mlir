...
func.func @main() -> i32 {
  ...
  //===--------------------------------------------------------------------===//
  // VP Intrinsic FMin Reduce Operation + Fixed Vector Type
  //===--------------------------------------------------------------------===//

  // Mask-Driven
  %vec1 = vector.load %mem_f32[%c0] : memref<20xf32>, vector<8xf32>
  %res_fmin_mask_driven = "llvm.intr.vp.reduce.fmin" (%c1_f32, %vec1, %mask6, %evl8) :
  ...
  %res_fmin_evl_driven = "llvm.intr.vp.reduce.fmin" (%c1_f32, %vec2, %mask8, %evl6) :
         (f32, vector<8xf32>, vector<8xi1>, i32) -> f32
  vector.print %res_fmin_evl_driven : f32

  //===--------------------------------------------------------------------===//
  // VP Intrinsic SMin Reduce Operation + Fixed Vector Type
  //===--------------------------------------------------------------------===//

  // Mask-Driven
  %vec3 = vector.load %mem_i32[%c0] : memref<20xi32>, vector<8xi32>
  %res_smin_mask_driven = "llvm.intr.vp.reduce.smin" (%c1_i32, %vec3, %mask6, %evl8) :
  ...
  %res_smin_evl_driven = "llvm.intr.vp.reduce.smin" (%c1_i32, %vec4, %mask8, %evl6) :
         (i32, vector<8xi32>, vector<8xi1>, i32) -> i32
  ...
  %res_umin_mask_driven = "llvm.intr.vp.reduce.umin" (%c1_i32, %vec5, %mask6, %evl8) :
         (i32, vector<8xi32>, vector<8xi1>, i32) -> i32
  vector.print %res_umin_mask_driven : i32

  // EVL-Driven
  %vec6 = vector.load %mem_i32[%c0] : memref<20xi32>, vector<8xi32>
  %res_umin_evl_driven = "llvm.intr.vp.reduce.umin" (%c1_i32, %vec6, %mask8, %evl6) :
         (i32, vector<8xi32>, vector<8xi1>, i32) -> i32
