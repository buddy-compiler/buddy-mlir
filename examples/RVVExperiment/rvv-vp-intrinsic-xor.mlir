...
func.func @main() -> i32 {
  ...
  %res_evl_driven = "llvm.intr.vp.xor" (%vec3, %vec4, %mask8, %evl6) :
         (vector<8xi32>, vector<8xi32>, vector<8xi1>, i32) -> vector<8xi32>
  vector.print %res_evl_driven : vector<8xi32>

  //===--------------------------------------------------------------------===//
  // VP Intrinsic Reduce XOR Operation + Fixed Vector Type
  //===--------------------------------------------------------------------===//

  // Mask-Driven
  %vec5 = vector.load %mem[%c0] : memref<20xi32>, vector<8xi32>
  %res_reduce_mask_driven = "llvm.intr.vp.reduce.xor" (%c1_i32, %vec5, %mask6, %evl8) :
  ...
  %res_reduce_evl_driven = "llvm.intr.vp.reduce.xor" (%c1_i32, %vec6, %mask8, %evl6) :
         (i32, vector<8xi32>, vector<8xi1>, i32) -> i32
