...
//
// This file tests lowering a linalg.generic matmul pattern to BOSCAME.

#matmul_a = affine_map<(d0, d1, d2) -> (d0, d2)>
#matmul_b = affine_map<(d0, d1, d2) -> (d2, d1)>
#matmul_c = affine_map<(d0, d1, d2) -> (d0, d1)>

module {
  func.func private @print_C(i32, i32, i32, i32)

  func.func @generic_matmul_i32_4x4x4(%A: memref<4x4xi32>, %B: memref<4x4xi32>,
                                      %C: memref<4x4xi32>) {
    linalg.generic {
      indexing_maps = [#matmul_a, #matmul_b, #matmul_c],
  ...
  func.func @main() -> i32 {
    ...
    call @generic_matmul_i32_4x4x4(%A, %B, %C)
...
// CHECK-LABEL: func.func @generic_matmul_i32_4x4x4
