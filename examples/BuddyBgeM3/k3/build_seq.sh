#!/usr/bin/env bash
# ===- build_seq.sh - Compile one seq variant of BGE-M3 RAX on K3 ---------===//
#
...
# ===----------------------------------------------------------------------===//
#
# Builds one seq variant (128 | 256 | 512) of bge_m3.rax natively on K3.
# Pure shell, no python / CMake / torch: only prebuilt LLVM + buddy tools.
#
# Usage:
#   bash build_seq.sh <seq> [tag]
#     seq : 128 | 256 | 512
#     tag : output dir suffix, default "baseline". Use a new tag for
#           optimization experiments, never overwrite the baseline.
#
#   e.g. bash build_seq.sh 128               -> out/seq128-baseline/
#        bash build_seq.sh 128 exp1_scalable -> out/seq128-exp1_scalable/
#
# Prerequisites:
#   $SRC/dist/seq<seq>/ has forward.mlir / subgraph0.mlir / arg0.data /
#   tokenizer.json (transferred from x86), and build_runtime.sh has run.
#
# Optimization knob: edit SUB_PASSES below and rerun this script.
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$HERE/env.sh"

SEQ=${1:?Usage: build_seq.sh <seq> [tag]}
TAG=${2:-baseline}
SRC="$APP/src/dist/seq$SEQ"  # MLIR graphs for this seq
SHARED="$APP/src/dist/"       # shared big files: arg0.data / tokenizer.json
OUTD="$OUT/seq$SEQ-$TAG"
RT="$OUT/runtime"

# Shared file lookup: prefer the seq dir, then the dist root.
res() {
  local p
...
#   and delete the "-convert-vector-to-scf" line.
# Experiment B (on top of A, add batch matmul):
#     -batchmatmul-transpose-b-vectorization=vector-type=scalable vector-size=4
#     -batchmatmul-optimize
# Experiment C (layout assumption check: keep the original pass, add the
#   layout triple only).
# ══════════════════════════════════════════════════════════════════════════
...
# Manifest: prefer the x86 pre-generated one; otherwise generate on the fly
# with system python3 (pure stdlib).
