...
// TileMatMulOp → convert-tile-to-buckyball → bb_matmul (per subview tile) → …
// Single-tile case: 16×16 × 16×16 so one `bb_matmul` (K not split across tiles).
// GRM: pure MLIR (same as buckyball.c cpu_matmul). DUT: `tile.tile_matmul`.
// Host: debug_helper.c -> bb_test_report.
// CHECK: bb_mul_warp16

"memref.global"() {sym_name = "a_g", type = memref<16x16xi8>, initial_value = dense<1> : tensor<16x16xi8>, visibility = "private"} : () -> ()
"memref.global"() {sym_name = "b_g", type = memref<16x16xi8>, initial_value = dense<1> : tensor<16x16xi8>, visibility = "private"} : () -> ()

func.func private @bb_test_report(i32) -> ()

// -----------------------------------------------------------------------------
// GRM — golden reference (same as buckyball.c cpu_matmul, L246–256), K=N=16
// -----------------------------------------------------------------------------
func.func @grm_matmul_ref_16(%a : memref<16x16xi8>, %b : memref<16x16xi8>, %c : memref<16x16xi32>) {
  ...
  scf.for %i = %c0 to %n step %c1 {
    scf.for %j = %c0 to %n step %c1 {
      ...
      %dot = scf.for %k = %c0 to %n step %c1 iter_args(%acc = %z) -> (i32) {
        %va = memref.load %a[%i, %k] : memref<16x16xi8>
        %vb = memref.load %b[%k, %j] : memref<16x16xi8>
...
func.func @grm_cmp_i32_16x16(%g : memref<16x16xi32>, %d : memref<16x16xi32>) -> i1 {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %n = arith.constant 16 : index
...
func.func @main() -> i8 {
  ...
  %a = memref.get_global @a_g : memref<16x16xi8>
  %b = memref.get_global @b_g : memref<16x16xi8>
  %c_grm = memref.alloc() : memref<16x16xi32>
  %c_dut = memref.alloc() : memref<16x16xi32>

  func.call @grm_matmul_ref_16(%a, %b, %c_grm) : (memref<16x16xi8>, memref<16x16xi8>, memref<16x16xi32>) -> ()

  "tile.tile_matmul"(%a, %b, %c_dut) : (memref<16x16xi8>, memref<16x16xi8>, memref<16x16xi32>) -> ()
