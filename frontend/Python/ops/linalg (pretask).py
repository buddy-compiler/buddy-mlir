...
#修改后的函数将输入节点的矩阵乘法结果与第三个输入张量相加。这里假设输入节点的参数数量为3，其中前两个参数是需要进行矩阵乘法的张量，第三个参数是要与矩阵乘法结果相加的张量。
def matadd_op(
    node: torch.fx.Node,
    symbol_table: Dict[Tuple[str, int], ir.Operation],
):
    """
    Import the tensor matrix addition operation.
    From PyTorch `aten.addmm` operator to MLIR linalg `matmul` operation.

    Note: This op, compute input node's matrix addition result.
    Args:
        node: Containing information from the input graph node.
        symbol_table: A dictionary mapping symbols to their corresponding
        operations.
    ...
    if dtype == "torch.float32":
        ...
        attr = ir.DenseElementsAttr.get_splat(tensor_type, element)
        matadd_result_buffer = arith.ConstantOp(tensor_type, attr).result
        op = linalg.matmul(input1, input2, outs=[matadd_result_buffer])
        op = arith.AddFOp(op, input3)
