    ...
    def lower_to_llvm_ir(self):
        ...
        with ir.Location.unknown(self._ctx):
            ...
            if(self.device == DeviceType.GPU):
                ...
                pm.add("-canonicalize")
                pm.add("-gpu-map-parallel-loops")
                pm.add("-convert-parallel-loops-to-gpu")
                pm.add("-gpu-kernel-outlining")
                pm.add("-buffer-deallocation")
                pm.add("-canonicalize")
                pm.add("-cse")
                pm.add("-convert-memcpy-to-gpu='process-args=0'")
                pm.add("-gpu-async-region")
                pm.add("-canonicalize")
                pm.add("-llvm-request-c-wrappers")
                pm.add("--test-lower-to-nvvm='cubin-chip=sm_80 cubin-features=+ptx71 cubin-format=fatbin'")
            elif(self.device == DeviceType.CPU):
                pm.add("arith-expand")
