...
created = [
    value.name for value in fx_nodes if not _is_unsupported_get_attr(value)
]
...
assert all("kwargs_old" not in value._children for value in graph.body)
...
transpose_matmul_fusion(
    graph, mm, permute, [left, permute], "transpose_matmul_fusion"
)
...
k_unsqueeze = node(
    UnsqueezeOp, "k_unsqueeze", ["k_index"], ["k_index"], ["k_expand"]
)
k_expand = node(
    ExpandOp, "k_expand", ["k_unsqueeze"], ["k_unsqueeze"], ["k_clone"]
)
k_clone = node(CloneOp, "k_clone", ["k_expand"], ["k_expand"], ["k_view"])
k_view = node(ViewOp, "k_view", ["k_clone"], ["k_clone"], ["sdpa"])
v_unsqueeze = node(
    UnsqueezeOp, "v_unsqueeze", ["v_index"], ["v_index"], ["v_expand"]
)
v_expand = node(
    ExpandOp, "v_expand", ["v_unsqueeze"], ["v_unsqueeze"], ["v_clone"]
)
...
k_unsqueeze2 = node(
    UnsqueezeOp, "k_unsqueeze2", ["k_index2"], ["k_index2"], ["k_expand2"]
)
k_expand2 = node(
    ExpandOp,
    "k_expand2",
...
v_unsqueeze2 = node(
    UnsqueezeOp, "v_unsqueeze2", ["v_index2"], ["v_index2"], ["v_expand2"]
)
v_expand2 = node(
    ExpandOp, "v_expand2", ["v_unsqueeze2"], ["v_unsqueeze2"], ["v_clone2"]
)
...
gqa = graph.node_table["GQAAttentionFusedOp_1"]
assert (
    k_expand2 in graph.body
    and k_unsqueeze2 in graph.body
    and k_index2 in graph.body
)
...
assert (
    multi_a.args[0] == replacement.name and multi_b.args[0] == replacement.name
)
...
unsqueeze = node(
    UnsqueezeOp, "unsqueeze", ["unsq_transpose", 0], ["unsq_transpose"]
)
...
reshape = node(
    ReshapeOp, "skip_reshape", ["skip_transpose", [2, 1]], ["skip_transpose"]
)
reshape.tensor_meta = {"shape": (2, 1), "dtype": TensorDType.Float32}
...
assert reshape._source_meta == meta("skip_view") + meta(
    "skip_transpose"
) + meta("skip_reshape")
