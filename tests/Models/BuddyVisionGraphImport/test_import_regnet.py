...
if build_dir:
    output_dir = Path(build_dir) / "tests/Models/BuddyVisionGraphImport/regnet"
else:
    repo_root = script_dir.parent.parent.parent
    output_dir = repo_root / "build/tests/Models/BuddyVisionGraphImport/regnet"
output_dir.mkdir(parents=True, exist_ok=True)

# Retrieve model path from environment variable
model_path = os.environ.get("REGNET_MODEL_PATH")
if model_path is None:
    model_path = "facebook/regnet-y-040"

print(f"Loading RegNet model config from: {model_path}")
...
model = RegNetForImageClassification(config).eval()
...
print("✓ RegNet graph construction test PASSED")
