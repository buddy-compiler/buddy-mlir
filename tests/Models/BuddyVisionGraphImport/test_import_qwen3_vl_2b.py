...
# ===---------------------------------------------------------------------------
#
# This is the graph coverage test for Qwen3-VL-2B-Instruct model.
#
# ===---------------------------------------------------------------------------
...
parser = argparse.ArgumentParser(description="Qwen3-VL-2B graph coverage test")
parser.add_argument(
    "--output-dir",
    type=str,
    default=None,
    help="Directory to save output MLIR files (default: build/tests/Models/BuddyVisionGraphImport/qwen3_vl_2b)",
)
...
if args.output_dir is None:
    ...
    if build_dir:
        output_dir = (
            Path(build_dir) / "tests/Models/BuddyVisionGraphImport/qwen3_vl_2b"
        )
    else:
        repo_root = script_dir.parent.parent.parent
        output_dir = (
            repo_root / "build/tests/Models/BuddyVisionGraphImport/qwen3_vl_2b"
        )
else:
...
model_path = os.environ.get("QWEN3_VL_2B_MODEL_PATH")
if model_path is None:
    model_path = "Qwen/Qwen3-VL-2B-Instruct"

print(f"Loading Qwen3-VL-2B-Instruct model from: {model_path}")
...
# Note: Complex models may produce operations (like tensor.extract_slice,
# linalg.generic, math.fpowi) that print in generic form, causing the entire
# module to use generic operation form printing. The generic form uses
# sym_name = "subgraph0" instead of func.func @subgraph0.
mlir_str = str(driver.subgraphs[0]._imported_module)
assert 'sym_name = "subgraph0"' in mlir_str, 'Missing sym_name = "subgraph0"'
...
print("✓ Qwen3-VL-2B-Instruct graph construction test PASSED")
