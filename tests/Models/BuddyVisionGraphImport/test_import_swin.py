...
if build_dir:
    output_dir = Path(build_dir) / "tests/Models/BuddyVisionGraphImport/swin"
else:
    repo_root = script_dir.parent.parent.parent
    output_dir = repo_root / "build/tests/Models/BuddyVisionGraphImport/swin"
output_dir.mkdir(parents=True, exist_ok=True)

# Retrieve model path from environment variable
model_path = os.environ.get("SWIN_MODEL_PATH")
if model_path is None:
    model_path = "microsoft/swin-tiny-patch4-window7-224"

print(f"Loading Swin model config from: {model_path}")
...
model = SwinForImageClassification(config).eval()
...
print("✓ Swin graph construction test PASSED")
