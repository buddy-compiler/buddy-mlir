...
# ===---------------------------------------------------------------------------
#
# This is the graph coverage test for SmolVLM-256M-Instruct model.
#
# ===---------------------------------------------------------------------------
...
parser = argparse.ArgumentParser(
    description="SmolVLM-256M-Instruct graph coverage test"
)
parser.add_argument(
    "--output-dir",
    type=str,
    default=None,
    help="Directory to save output MLIR files (default: build/tests/Models/BuddyVisionGraphImport/smolvlm_256m)",
)
...
if args.output_dir is None:
    ...
    if build_dir:
        output_dir = (
            Path(build_dir) / "tests/Models/BuddyVisionGraphImport/smolvlm_256m"
        )
    else:
        repo_root = script_dir.parent.parent.parent
        output_dir = (
            repo_root / "build/tests/Models/BuddyVisionGraphImport/smolvlm_256m"
        )
else:
...
model_path = os.environ.get("SMOLVLM_256M_MODEL_PATH")
if model_path is None:
    model_path = "HuggingFaceTB/SmolVLM-256M-Instruct"

print(f"Loading SmolVLM-256M-Instruct model from: {model_path}")
...
print("✓ SmolVLM-256M-Instruct graph construction test PASSED")
