...
if build_dir:
    output_dir = Path(build_dir) / "tests/Models/BuddyVisionGraphImport/deit"
else:
    repo_root = script_dir.parent.parent.parent
    output_dir = repo_root / "build/tests/Models/BuddyVisionGraphImport/deit"
output_dir.mkdir(parents=True, exist_ok=True)

# Retrieve model path from environment variable
model_path = os.environ.get("DEIT_MODEL_PATH")
if model_path is None:
    model_path = "facebook/deit-base-distilled-patch16-224"

print(f"Loading DeiT model config from: {model_path}")
...
print("✓ DeiT graph construction test PASSED")
