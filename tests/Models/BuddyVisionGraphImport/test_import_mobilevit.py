...
if build_dir:
    output_dir = (
        Path(build_dir) / "tests/Models/BuddyVisionGraphImport/mobilevit"
    )
else:
    repo_root = script_dir.parent.parent.parent
    output_dir = (
        repo_root / "build/tests/Models/BuddyVisionGraphImport/mobilevit"
    )
output_dir.mkdir(parents=True, exist_ok=True)

# Retrieve model path from environment variable
model_path = os.environ.get("MOBILEVIT_MODEL_PATH")
if model_path is None:
    model_path = "apple/mobilevit-small"

print(f"Loading MobileViT model config from: {model_path}")

config = MobileViTConfig(
    image_size=64,
    num_channels=3,
    hidden_sizes=[16, 32, 64],
    neck_hidden_sizes=[16, 32, 64, 96, 128, 160, 192],
    depths=[1, 1, 1],
    num_labels=10,
)
...
print("✓ MobileViT graph construction test PASSED")
