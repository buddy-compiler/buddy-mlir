...
if build_dir:
    output_dir = Path(build_dir) / "tests/Models/BuddyLLMGraphImport/electra"
else:
    repo_root = script_dir.parent.parent.parent
    output_dir = repo_root / "build/tests/Models/BuddyLLMGraphImport/electra"
output_dir.mkdir(parents=True, exist_ok=True)

# Retrieve model path from environment variable
model_path = os.environ.get("ELECTRA_MODEL_PATH")
if model_path is None:
    model_path = "google/electra-small-discriminator"

print(f"Loading ELECTRA model config from: {model_path}")
...
model = ElectraModel(config).eval()
...
print("✓ ELECTRA graph construction test PASSED")
