...
if build_dir:
    output_dir = (
        Path(build_dir) / "tests/Models/BuddyLLMGraphImport/xlm_roberta"
    )
else:
    repo_root = script_dir.parent.parent.parent
    output_dir = (
        repo_root / "build/tests/Models/BuddyLLMGraphImport/xlm_roberta"
    )
output_dir.mkdir(parents=True, exist_ok=True)

# Retrieve model path from environment variable
model_path = os.environ.get("XLM_ROBERTA_MODEL_PATH")
if model_path is None:
    model_path = "FacebookAI/xlm-roberta-base"

print(f"Loading XLM-RoBERTa model config from: {model_path}")
...
model = XLMRobertaModel(config).eval()
...
print("✓ XLM-RoBERTa graph construction test PASSED")
