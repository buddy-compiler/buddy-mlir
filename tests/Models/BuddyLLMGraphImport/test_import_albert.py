...
if build_dir:
    output_dir = Path(build_dir) / "tests/Models/BuddyLLMGraphImport/albert"
else:
    repo_root = script_dir.parent.parent.parent
    output_dir = repo_root / "build/tests/Models/BuddyLLMGraphImport/albert"
output_dir.mkdir(parents=True, exist_ok=True)

# Retrieve model path from environment variable
model_path = os.environ.get("ALBERT_MODEL_PATH")
if model_path is None:
    model_path = "albert/albert-base-v2"

print(f"Loading ALBERT model config from: {model_path}")
...
print("✓ ALBERT graph construction test PASSED")
