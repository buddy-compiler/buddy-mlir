...
if build_dir:
    output_dir = Path(build_dir) / "tests/Models/BuddyLLMGraphImport/opt"
else:
    repo_root = script_dir.parent.parent.parent
    output_dir = repo_root / "build/tests/Models/BuddyLLMGraphImport/opt"
output_dir.mkdir(parents=True, exist_ok=True)

# Retrieve model path from environment variable
model_path = os.environ.get("OPT_MODEL_PATH")
if model_path is None:
    model_path = "facebook/opt-125m"

print(f"Loading OPT model config from: {model_path}")
...
print("✓ OPT graph construction test PASSED")
