...
if build_dir:
    output_dir = Path(build_dir) / "tests/Models/BuddyLLMGraphImport/pythia"
else:
    repo_root = script_dir.parent.parent.parent
    output_dir = repo_root / "build/tests/Models/BuddyLLMGraphImport/pythia"
output_dir.mkdir(parents=True, exist_ok=True)

# Retrieve model path from environment variable
model_path = os.environ.get("PYTHIA_MODEL_PATH")
if model_path is None:
    model_path = "EleutherAI/pythia-70m"

print(f"Loading Pythia model config from: {model_path}")
...
print("✓ Pythia graph construction test PASSED")
