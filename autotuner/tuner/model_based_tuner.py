...
class CostModel(object):
    ...
    def fit_log(self, records, plan_size, min_seed_records=500):
        ...
        Parameters
        ----------
        records: Array of Tuple(config: MeasureInput, result: MeasureResult)
            The tuning records
        plan_size: int
            The plan size of tuner
...
class ModelBasedTuner(Tuner):
    ...
    cost_model: CostModel
        The cost model that predicts the speed of a config
    model_optimizer:
    ...
    def has_next(self):
        return len(self.visited) < self.space.range_length
