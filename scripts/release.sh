...
PY_TAG="${1:?Error: Python ABI (parameter 1, format \"cp312-cp312\") is required but not set.}"
...
if [ "${PY_TAG}" != "cp312-cp312" ]; then
  echo "Error: only cp312-cp312 is supported for abi3 release builds." >&2
  exit 1
fi

PYTHON_BUILD_TAG="${PY_TAG}"
PYTHON_WHEEL_TAG="cp312-abi3"
...
EOF
  ...
  export BUDDY_PY_LIMITED_API="cp312"
