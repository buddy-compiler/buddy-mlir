...
class GEMMVectorizationPattern : public ConversionPattern {
public:
  explicit GEMMVectorizationPattern(MLIRContext *context,
	        ...
	        buildAffineLoopNest(builder, loc, {c0}, {K}, 1, [&](OpBuilder& builder, Location loc, ValueRange ivRange){
				...
				for(int i = 0; i < kernelM; ++ i){
					...
					for(int j = 0; j < kernelN; ++ j){
						Value fixedIV = builder.create<AffineApplyOp>(loc, AffineMap::get(1, 0, d0 + j * strip), ivJ);
				...
				for(int i = 0; i < kernelN; ++ i){
					Value fixedIV = ivJ;
					if(i != 0){
						fixedIV = builder.create<AffineApplyOp>(loc, AffineMap::get(1, 0, d0 + i * strip), ivJ);
					}
...
//===----------------------------------------------------------------------===//
// GEMMVectorizationPass
//===----------------------------------------------------------------------===//

/// This is a partial lowering linalg pooling operations to mixture of
/// Affine + Vector operations.
namespace {
class GEMMVectorizationPass
    : public PassWrapper<GEMMVectorizationPass, OperationPass<ModuleOp>> {
public:
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(GEMMVectorizationPass)
  StringRef getArgument() const final { return "gemm-vectorization"; }
  StringRef getDescription() const final { return "GEMM vectorization."; }
  GEMMVectorizationPass() = default;
  GEMMVectorizationPass(const GEMMVectorizationPass &) {}
  explicit GEMMVectorizationPass(int64_t stripParam, int64_t kernelMParam, int64_t kernelNParam) { strip = stripParam; kernelM = kernelMParam; kernelN = kernelNParam; }
...
void GEMMVectorizationPass::runOnOperation() {
  ...
  patterns.add<GEMMVectorizationPattern>(context, strip, kernelM, kernelN);
...
void registerGEMMVectorizationPass() {
  PassRegistration<GEMMVectorizationPass>();
}
} // namespace buddy
} // namespace mlir
