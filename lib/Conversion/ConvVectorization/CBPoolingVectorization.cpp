...
public:
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> /*operands*/,
    ...
    auto strides = op->getAttrOfType<mlir::DenseIntElementsAttr>("strides")
                       .getValues<int64_t>();
    Value stridesAttr = rewriter.create<arith::ConstantOp>(
        loc, op->getAttrOfType<mlir::DenseIntElementsAttr>("strides"));
    Value stridesHeightI64 =
        rewriter.create<tensor::ExtractOp>(loc, stridesAttr, c0);
    Value stridesHeight = rewriter.create<arith::IndexCastOp>(
        loc, rewriter.getIndexType(), stridesHeightI64);
    Value stridesWidthI64 =
        rewriter.create<tensor::ExtractOp>(loc, stridesAttr, c1);
    Value stridesWidth = rewriter.create<arith::IndexCastOp>(
        loc, rewriter.getIndexType(), stridesWidthI64);
    // Dilations.
    // TODO Dilated pooling.
